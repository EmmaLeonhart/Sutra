...
- **Opinionated, not authoritarian.** Sutra is Turing-complete; you *can* write unbounded loops, non-tail recursion, and other patterns hostile to the substrate (GPUs, neuromorphic chips, fly-brain). The compiler will warn loudly and explain *why* the pattern hurts on the intended target — but it lets you compile. The escape hatches (legitimate cast / unsafe cast / force override; see `examples/uncertain/03-types-and-casts.su`) exist for the same reason: the language doesn't pretend the bad path doesn't exist, it just makes choosing it explicit and grep-able.
