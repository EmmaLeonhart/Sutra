...
- **OCaml** (`sutra-from-ocaml/`, reference): `option`/variant payload support is **DONE 2026-06-19**
  ...
  - **Multibase NON-tail recursion — DONE 2026-06-20 across all four (OCaml/Scala/F#/Rust).** A
    parity check after clearing the Elixir/Erlang/Haskell multibase-non-tail items found OCaml, Scala,
    F#, AND Rust all `UNSUPPORTED` a ≥2-base non-tail recursion (`f a b = if a==0 then b else if a==1
    then b+100 else a + f (a-1) b`). Ported the fold recipe to all four (`_try_lower_multibase_nontail*`;
    fixtures `multiarg_nontail_multibase` RUN == 115 each). Per-frontend wrinkles: OCaml/Rust nested
    `if/else if`, Scala single-line `if/else if` (multi-line body is an `indented_block`), F#'s `elif`
    is a flat `elif_expression` child + the self-call is parenthesised per F# convention. Finding
    `2026-06-19-multibase-nontail-gap-ocaml-scala-fsharp-rust.md` RESOLVED.
