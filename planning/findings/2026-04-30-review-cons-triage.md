...
**What's actually going on:** `max_iters=50` is a default parameter in the `loop()` method (`codegen_pytorch.py:1118`). It's a compiler parameter, not a fundamental architectural limitation. The loop is a soft-halt RNN: each step applies `state ← R * state`, projects through cleanup, and accumulates a halt signal via sigmoid. The loop *converges* (halted saturates to 1.0) when the state matches a compiled prototype. T=50 is just how many unroll steps the compiler emits.
