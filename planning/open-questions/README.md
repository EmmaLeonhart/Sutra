...
- `numpy-inheriting-from-flybrain.md` — NumpyCodegen inherits from FlyBrainCodegen, which contradicts CLAUDE.md's "numpy is the demo path, fly-brain is segregated" framing. Refactor: move the backend-agnostic translator body into a new `codegen_base.py`; make fly-brain and numpy sibling backends. Deferred — not wrong output, just wrong framing.
