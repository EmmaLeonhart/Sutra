...
- `meta-matrix-concept-to-projector.md` — is there a single learned `is_converter` matrix such that `is_converter @ c` produces the `is_c` test matrix for any concept `c`? Would amortize the per-relation fitting story in `binding.md` to one fit. Empirically untested; a research bet, not a current commitment.
