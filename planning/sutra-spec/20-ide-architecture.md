...
Sutra's IDE is not optional tooling. Because the language has long-range semantic dependencies that no single source file can capture (see [06-runtime.md](06-runtime.md)), the IDE is part of the language — the same way TypeScript's type checker is a second interpreter running alongside the code. This document describes the vision for that IDE.

Status: **v0.3 with the workspace system landed on top of v0.2 under `sdk/intellij-sutra/`.** v0.1 gave us file type registration, hand-written lexer, syntax highlighter, color settings page, brace matcher, commenter, quote handler, keyword/primitive/builtin completion, live templates ported from the VS Code extension, and an external annotator that shells out to `python -m sutra_compiler --json`. v0.2 added a persistent `SutraSettings` service and a proper **Settings → Tools → Sutra** `Configurable` that replaces the env-var-only v0.1 configuration (env vars remain a fallback), JUnit 4 unit tests for the hand-written lexer covering every token category, a design-only Kotlin interface anchor for the MCP tool surface named in §"Minimum Agent Workflows" below (`SutraMcpSurface` + stub), and the two visualizer tool windows (embedding-space scatter + fly-brain topological view, both via JCEF with Canvas 2D rendering). v0.3 adds the **workspace system**: `atman.toml` workspace file type, a reference Python parser at `sdk/sutra-compiler/sutra_compiler/solution.py` with 17 unit tests covering the SUT2000-SUT2099 error range, a new **Sutra Workspace** tool window on the left side that scans the project for a `atman.toml` workspace file, shells out to the Python parser, and renders the solution structure as a `JTree` of projects and source files with double-click-to-open. The format spec is in [`22-workspaces.md`](22-workspaces.md). What remains design-only: real PSI parser, MCP implementation, debugger integration, `ProjectOpenProcessor` for auto-detect on folder open, and source-root configuration via `ModuleRootModificationUtil`.

## Non-Goal: Generic Editor With Sutra Support

Sutra is explicitly *not* targeting a VS Code extension as its primary development environment. Two reasons:
...
- **Built-in MCP server support.** JetBrains ships an MCP server as part of IntelliJ IDEA, exposing the PSI tree, project model, and editor state to external AI agents. This is the single most important reason to pick IntelliJ. **AI agents will write most Sutra code** — the language's long-range semantic dependencies and fuzzy vector primitives are exactly the kind of thing humans will delegate — and IntelliJ's MCP integration gives us agent authoring out of the box, not as a project we have to build. See the dedicated section below.
...
- **Integrated debugger framework.** The debugger UI and stepping model are already wired; Sutra only has to implement the domain-specific parts (stepping through embedding operations, inspecting intermediate vectors).
...
**Why Visual Studio.** Concrete features that map well to Sutra's model:

- **Solution file with multiple projects.** Separation of concerns lives at the *project* level, not the file level. Each Sutra project within a solution is a clean unit: one project for embedding operations, one for models, one for visualizations, with explicit dependency arrows between them.
...
|-------------------------|------------------------------------------------|-------------------|
| Language + compiler     | Sutra reference implementation                | No                |
...
| Visualizer              | 2D linear projection onto user-chosen composite basis (optional 3D mode + UMAP/t-SNE) | Yes |
| MCP server              | Local, auto-started by the IDE                 | Yes               |
| Project system          | Solution-based multi-project layout            | No                |

**The vector-database problem.** Most vector databases assume production infrastructure. Pinecone, Weaviate, Qdrant all assume a server context; getting a beginner to spin up one of those before they can print "hello world" is the same as asking them to set up PostgreSQL first. Sutra needs the SQLite equivalent — embedded, zero-config, just there. This is a required component of the stack; it may well become its own project.

**The zero-config default.** If the user doesn't choose an embedding space, Sutra picks one of the bundled ones and keeps going. This is the single most important affordance for learning — the onboarding friction of modern ML tooling is what keeps embedding-space programming out of reach for most people.
...
Embedding spaces are mathematically high-dimensional, but humans need a handle on what they are navigating. The IDE ships with a visualizer pane that shows a 2D projection of the current embedding space by default, with the vectors your code is currently touching highlighted.

Not because a linear 2D projection is mathematically complete — it obviously isn't — but because seeing where your code is operating in the space is the single most useful debugging affordance for this paradigm. This is genuinely unprecedented as a development experience; no existing IDE has a first-class "here is your computational substrate, rendered" panel.

### Default projection: 2D plane with user-chosen composite-vector basis

**The default is not UMAP or t-SNE.** It's a linear projection onto two composite vectors the programmer (or an agent) picks. Each axis is itself an Sutra expression — `male - female`, `king - peasant`, or any other bound name — and the visualizer simply renders the dot products. Two dot products per point, trivially redraws at 60fps, scales up to 3D by adding a third axis on demand.
...
- **Semantically meaningful.** Picking basis vectors out of the space itself turns the visualizer into a navigable semantic coordinate system you can reason about in the same vocabulary as your code — not just a pretty picture. This matches Sutra's "computation is geometry" pitch.
- **Composable with code.** The axes are first-class Sutra expressions. Code can reference the visualizer's current basis; the visualizer can bind to names computed by the code. UMAP can't do that — its projection isn't a first-class object.
- **Agent-visible.** Because the basis is just two (or three) vectors, the MCP surface exposes it as two/three `vector` values plus a `project(vector) -> (x, y)` or `project(vector) -> (x, y, z)` tool depending on mode. Agents can set the basis, query projected coordinates, and animate between bases the same way humans can.
- **Cheap.** Two dot products per point at the default, three in 3D mode. Rendering runs inside JCEF via three.js (see below) so hardware acceleration is on the table even before we have to reach for native rendering.

**2D is the default; 3D is opt-in.** Flipped from an earlier draft of this doc that defaulted to 3D. The reasons to default to 2D:

- **Simpler to read at a glance.** A 2D scatter is trivially interpretable — x and y axis, eye tracks the cloud. 3D adds orbit controls, perspective, and a depth cue the user has to learn to read before the viz pays off.
- **Cheaper to render.** Even with JCEF + three.js handling the work, a 2D orthographic projection is categorically cheaper than a 3D perspective one on large point clouds, and the point-count ceiling is the binding constraint for whether the pane is usable on real data.
- **Easier to screenshot, print, and paste.** 2D plots round-trip through documentation and paper figures without losing orientation context.
- **3D on demand.** A third composite basis vector is one control away, and the mode switch is a dropdown — 2D vs 3D vs UMAP vs t-SNE. Users who want the depth cue get it; users who don't aren't paying for it.
...
### Renderer: JCEF + three.js

**Decision:** render the visualizer inside a JCEF (JetBrains Chromium Embedded Framework) tool window, with three.js handling the actual drawing. Earlier drafts of this doc framed "JCEF + three.js vs native Swing with a batched renderer" as an open question; it's now resolved in favor of JCEF + three.js.

Rationale:

- **Web 3D ecosystem reuse.** three.js has orbit controls, line materials, point sprites, custom shaders, WebGL hardware acceleration, and a decade of rendering tricks for this exact kind of scatter-plus-edges scene. Reimplementing any of that in Swing/Graphics2D would be a multi-week side quest.
- **Agent surface is free.** A JCEF-hosted page exposes a JS-Kotlin bridge natively, so the MCP project/inspect calls that need to query the current rendered state (what's under the cursor, what's visible, current camera) have a clean path without building a custom serialization layer.
...
Sutra's MCP story is the hinge of the whole IDE design. There are actually **two** MCP servers involved, and they serve different audiences:

1. **The IntelliJ-provided MCP server** (shipped with IntelliJ IDEA) — exposes the IDE itself to AI agents: PSI tree, open files, project model, diagnostics, run configurations, debugger state. This is how agents *write* Sutra code.
2. **The Sutra runtime MCP server** ([06-runtime.md](06-runtime.md)) — exposes the embedding space itself: ANN lookups, entity resolution, cone/snap/hop primitives, the codebook. This is how Sutra code *runs*.
...
A key design assumption: **AI agents will write most Sutra code**, not humans. This isn't a hedge, it's the base case. Sutra has long-range semantic dependencies, fuzzy vector primitives, and an "instruction set" that is literally the geometry of a particular embedding space — the kind of code where human developers will instinctively delegate to an agent and review the output rather than write it by hand. The same forces that make Sutra need an IDE at all (semantics too rich for a single file to capture) also make it a language that rewards agent authoring.

This inverts the usual "IDE with optional AI help" framing. Sutra's IDE is an **agent-first authoring environment** where human editing is one of several input paths, not the default one.
...
"Vibe coding" Sutra — i.e. an agent iterating on code without a human in the tight loop — is an explicitly supported workflow, not an afterthought. Two agent loops in particular must be first-class, and both of them depend on MCP surfaces that most IDEs don't expose:

**1. The syntax-check loop.** The agent writes or edits Sutra code, asks the MCP server "is this well-formed and does it type-check against the current embedding space?", gets structured diagnostics back, and iterates. Required MCP tools:

- `sutra.parse(source) -> { ast, parse_errors }` — pure syntactic check, no space access needed
- `sutra.check(source, space_ref) -> { type_errors, probe_warnings, unresolved_entities }` — full semantic check against a target embedding space, including the empirical-initiation probe results
- `sutra.diagnostics(file) -> [...]` — same structured diagnostics the red-squiggle UI layer consumes, queryable at any time

The point of this loop is that the agent should almost never need to *run* Sutra code just to find out if it compiles. Syntax and type feedback is cheap, fast, and side-effect-free.

**2. The prototype-build loop.** The agent has a rough goal ("build an embedding-space classifier for these fly-brain cell types"), scaffolds a small Sutra program, compiles it, runs it against real vectors, inspects the output, and iterates. Required MCP tools:

- `sutra.scaffold(template, params) -> { project, files }` — bootstrap a new solution/project skeleton from a template, so the agent isn't rebuilding the project layout from scratch every time
- `sutra.compile(project) -> { artifact, compile_errors }` — drive the compiler directly, without going through the UI's run button
- `sutra.run(artifact, input) -> { output, intermediate_vectors, trace }` — execute a compiled artifact with instrumented output, so the agent gets not just the final vector but the intermediate steps, the S1/Sutra routing decisions, and any empirical-initiation corrections that fired
- `sutra.inspect(vector) -> { nearest, cone, magnitude, labels }` — query the runtime MCP server for "what does this vector mean in the current space," which is the agent's equivalent of a human dragging a point in the visualizer
...
- **How are the default basis vectors chosen when a programmer hasn't picked one?** Candidates: first two (or three, in 3D mode) PCA components of the loaded corpus, curated "semantic anchor" vectors shipped with the default map, or blank until the user picks. The visualizer has to have *something* to render on a cold open.
...
- **Scaffolding templates.** What's the starter set of `sutra.scaffold` templates for the prototype-build loop — classifier, similarity retriever, cone-traversal demo, fly-brain substrate program?

### Resolved

- **TextMate grammar as a day-one deliverable** → shipped. Lives at `sdk/vscode-sutra/syntaxes/sutra.tmLanguage.json` and is the source the IntelliJ plugin's hand-written lexer was ported from.
