...
During development of the FOL discovery system in this repo, a major defect was discovered in mxbai-embed-large (the 1024-dimensional embedding model used as Sutra's initial substrate). Diacritic characters — specifically macron vowels (o-macron, u-macron, etc.) used in Japanese romanization — cause **catastrophic embedding collapse**.
...
This is a **foundational risk** for Sutra. If the computational substrate has silent corruption modes, all computation built on it is unreliable in ways that are invisible without explicit checking.

Sutra must include:

1. **Substrate validation at empirical initiation time.** When Sutra probes a target embedding space, it should test for known pathologies (attention sinks, degenerate dimensions, collapse modes). A model that fails validation should be rejected as a substrate.
...
3. **Substrate abstraction.** Sutra programs should be substrate-agnostic — the same program runs on any embedding model that passes validation. If mxbai has a defect, swap to nomic or BGE without changing the program. This is already part of the design (empirical initiation produces different mappings for different models) but the pathology risk makes it load-bearing rather than just convenient.
