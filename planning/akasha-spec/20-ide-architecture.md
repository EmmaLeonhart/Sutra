...
Status: **v0.3 with the solution system landed on top of v0.2 under `sdk/intellij-akasha/`.** v0.1 gave us file type registration, hand-written lexer, syntax highlighter, color settings page, brace matcher, commenter, quote handler, keyword/primitive/builtin completion, live templates ported from the VS Code extension, and an external annotator that shells out to `python -m akasha_compiler --json`. v0.2 added a persistent `AkashaSettings` service and a proper **Settings → Tools → Akasha** `Configurable` that replaces the env-var-only v0.1 configuration (env vars remain a fallback), JUnit 4 unit tests for the hand-written lexer covering every token category, a design-only Kotlin interface anchor for the MCP tool surface named in §"Minimum Agent Workflows" below (`AkashaMcpSurface` + stub), and the two visualizer tool windows (embedding-space scatter + fly-brain topological view, both via JCEF with Canvas 2D rendering). v0.3 adds the **solution system**: `.aksln` / `.akproj` TOML file types, a reference Python parser at `sdk/akasha-compiler/akasha_compiler/solution.py` with 17 unit tests covering the AKA2000-AKA2099 error range, a new **Akasha Solution** tool window on the left side that scans the project for a `.aksln` file, shells out to the Python parser, and renders the solution structure as a `JTree` of projects and source files with double-click-to-open. The format spec is in [`22-solutions.md`](22-solutions.md). What remains design-only: real PSI parser, MCP implementation, debugger integration, `ProjectOpenProcessor` for auto-detect on folder open, and source-root configuration via `ModuleRootModificationUtil`.
