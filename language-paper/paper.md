...
- `bind(role, filler) = R · filler` — role-filler binding, where R is a matrix encoding the semantic role. A role in Sutra is a learned linear operator, not a random vector: "agent-of" is the matrix fit on (sentence, agent) embedding pairs; "is_cat" is the matrix fit on (entity, cat-label) pairs. This unifies binding with defuzzification — both are matrix-vector products, one encoding a semantic role and the other encoding a truth-extraction. The design target is matrices learned from relational displacement data in frozen embedding spaces; prior cartographic analysis discovered 86 predicates that manifest as consistent displacement vectors across three models (Leonhart, *Latent space cartography applied to Wikidata*), and displacements are the rank-0 (translation-only) special case of the full learned-matrix binding. The current demo runtime uses sign-flip binding (`a * sign(r)`, a self-inverse elementwise operation) as a placeholder on fresh random vectors, which is sufficient for the demonstrations; the transition to learned-matrix binding on real embedding substrates is in progress.
- `unbind(role, bound)` — the approximate inverse of `bind`. For the full matrix form, `R⁻¹ · bound` (or `R^T · bound` when R is orthogonal). For the sign-flip placeholder, identical to `bind`.
...
Sutra is a programming language in which `if`, `else`, `while`, `for`, `switch`, `break`, `continue`, and `return` do not exist. Their place is taken by `select`, `snap`, and `loop`, all compiled to vector operations. `select` is the one branching primitive — single-option (the `if`-shape, with a configurable fuzzy threshold defaulting to 0.5), multi-option (the `switch`-shape), and `else`-clause (the "none of the above" form) — and it never breaks differentiability because defuzzification is a polarization of the fuzzy state, not a binarization. The primitive set is computationally universal under standard VSA-completeness arguments. Binding is matrix-vector multiplication, where each role is a learned linear operator fit from relational data in the embedding substrate — unifying role-filler binding, truth-extraction, and defuzzification under a single matrix-vector primitive. A compiler, a specification, an IntelliJ plugin, a VS Code extension, and a numpy runtime exist and are in the public repository; three demonstration programs run end-to-end, producing 23/23 outputs matching their committed reference. The design point is the language's existence: a purely functional, Turing-complete programming surface that compiles to matmuls, sums, and cosines, with no machine branches anywhere in its runtime.
