...
/**
 * Registers the **Akasha Fly Brain** tool window — the v0.3 fly-brain
 ...
 *     blocked on the Akasha-unit ↔ hemibrain-neuron-ID mapping)
 *   - MCP surface for agent querying (flybrain.topology, flybrain.spikes
 *     etc.)
 *
 * Same JCEF-first pattern as [AkashaEmbeddingToolWindowFactory]:
 ...
 * Akasha runtime) slots in cleanly through the JS ↔ Kotlin bridge
 * JCEF already provides.
 */
class AkashaFlyBrainToolWindowFactory : ToolWindowFactory {
    ...
    override fun createToolWindowContent(project: Project, toolWindow: ToolWindow) {
        ...
        if (!JBCefApp.isSupported()) {
            ...
            try {
                ...
                LOG.warn("Failed to initialize JCEF for Akasha fly-brain visualizer", t)
    ...
    private fun buildJcefErrorPlaceholder(t: Throwable): JPanel {
        ...
        val label = JLabel(
            "<html><body style='width:480px'>" +
                "<h3>Akasha Fly Brain — initialization failed</h3>" +
    ...
    private fun buildFallbackHtml(message: String): String =
        "<html><body><h3>Akasha Fly Brain</h3><p>${escapeHtml(message)}</p></body></html>"
    ...
    companion object {
        private val LOG = Logger.getInstance(AkashaFlyBrainToolWindowFactory::class.java)
    }
}
