package org.akasha.intellij.mcp

/**
 * Interface surface for the Akasha MCP tools the IDE will eventually expose
 * to external agents.
 *
 * **Status: design-only.** This file is the *Kotlin-level anchor* for the
 * agent-authoring workflows described in
 * `planning/akasha-spec/20-ide-architecture.md` §"Minimum Agent Workflows
...
interface AkashaMcpSurface {
     ...
     *
     * Corresponds to the spec's `akasha.parse(source)` tool.
     */
     ...
     *
     * Corresponds to the spec's `akasha.check(source, space_ref)` tool.
     */
     ...
     * [org.akasha.intellij.AkashaExternalAnnotator] produces, just
     * reachable without going through the inspection pipeline.
     *
     * Corresponds to the spec's `akasha.diagnostics(file)` tool.
     */
     ...
     *
     * Corresponds to the spec's `akasha.scaffold(template, params)` tool.
     */
     ...
     *
     * Corresponds to the spec's `akasha.inspect(vector)` tool.
     */
