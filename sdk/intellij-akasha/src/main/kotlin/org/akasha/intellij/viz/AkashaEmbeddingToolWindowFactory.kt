package org.akasha.intellij.viz
...
/**
 * Registers the **Akasha Embedding Space** tool window — the v0.2
 * first-cut visualizer from
 * `planning/akasha-spec/20-ide-architecture.md` §"Embedding-Space
 ...
 *   - User-chosen composite basis vectors as Akasha expressions.
 ...
 *
 * See companion [AkashaFlyBrainToolWindowFactory] for the fly-brain
 * topological view, which uses the same pattern.
 */
class AkashaEmbeddingToolWindowFactory : ToolWindowFactory {
    ...
    override fun createToolWindowContent(project: Project, toolWindow: ToolWindow) {
        ...
        if (!JBCefApp.isSupported()) {
            ...
            try {
                ...
                LOG.warn("Failed to initialize JCEF for Akasha embedding visualizer", t)
    ...
    private fun buildJcefUnavailablePlaceholder(): JPanel {
        ...
        val label = JLabel(
            "<html><body style='width:480px'>" +
                ...
                "<code>planning/akasha-spec/20-ide-architecture.md</code> " +
    ...
    private fun buildJcefErrorPlaceholder(t: Throwable): JPanel {
        ...
        val label = JLabel(
            "<html><body style='width:480px'>" +
                "<h3>Akasha Embedding Space — initialization failed</h3>" +
    ...
    private fun buildFallbackHtml(message: String): String =
        "<html><body><h3>Akasha Embedding Space</h3><p>${escapeHtml(message)}</p></body></html>"
    ...
    companion object {
        private val LOG = Logger.getInstance(AkashaEmbeddingToolWindowFactory::class.java)
    }
}
