...
import org.akasha.intellij.AkashaSettings
...
/**
 * In-memory representation of an Akasha solution, mirroring the
 * Python data classes in `sdk/akasha-compiler/akasha_compiler/solution.py`.
 *
 * Loading is done by shelling out to the reference Python parser
 * via `python -m akasha_compiler.solution --json <path>` and
 * deserializing the JSON output. This is the same
 * shell-out-to-Python pattern [AkashaExternalAnnotator] already
 * uses for compiler diagnostics, so it reuses the same interpreter
 * configuration from [AkashaSettings] and does not introduce a new
 * dependency.
 *
 ...
 */
data class AkashaSolutionModel(
    val name: String,
    val akashaVersion: String,
    val description: String,
    val defaultSubstrate: String,
    val compilerArgs: List<String>,
    val aksFile: Path,
    val projects: List<AkashaProjectModel>,
) {
    val projectsByName: Map<String, AkashaProjectModel>
        get() = projects.associateBy { it.name }
}

data class AkashaProjectModel(
    val name: String,
    val path: Path,
    val entry: Path,
    ...
    val dependencies: List<AkashaProjectDependency>,
)

data class AkashaProjectDependency(
    val name: String,
    val path: Path,
)

/**
 * Either [Success] with a parsed [AkashaSolutionModel] or [Failure]
 * with a diagnostic string (typically the first line of the Python
 * parser's stderr, which carries the `AKA####` code).
 */
sealed class AkashaSolutionLoadResult {
    data class Success(val solution: AkashaSolutionModel) : AkashaSolutionLoadResult()
    data class Failure(val message: String) : AkashaSolutionLoadResult()
}
 ...
 */
object AkashaSolutionLoader {

    private val LOG = Logger.getInstance(AkashaSolutionLoader::class.java)
     ...
     */
    fun loadSolution(aksFile: Path, timeoutSeconds: Long = 15): AkashaSolutionLoadResult {
        ...
        if (rawArgs.isNotEmpty()) {
            ...
            // module-invocation target so `akasha_compiler.solution` is
            // the script, not the top-level `akasha_compiler` entry.
            // The rawArgs from settings is typically `-m akasha_compiler`;
            // we substitute `-m akasha_compiler.solution` to invoke the
            // solution-specific CLI.
            val substituted = rawArgs.replace(
                Regex("(^|\\s)-m\\s+akasha_compiler(\\b)"),
                "$1-m akasha_compiler.solution$2",
            )
        ...
        return try {
            ...
            if (!finished) {
                ...
                return AkashaSolutionLoadResult.Failure(
                    "solution parser timed out after ${timeoutSeconds}s"
                )
            }
            if (process.exitValue() != 0) {
                val firstErrLine = stderr.lineSequence().firstOrNull() ?: "unknown error"
                return AkashaSolutionLoadResult.Failure(firstErrLine.trim())
            }
            AkashaSolutionLoadResult.Success(parseJson(stdout, aksFile))
        } catch (e: Exception) {
            LOG.warn("Failed to invoke solution parser for $aksFile", e)
            AkashaSolutionLoadResult.Failure("failed to invoke solution parser: ${e.message}")
        }
    }

    private fun parseJson(json: String, aksFile: Path): AkashaSolutionModel {
        ...
        return AkashaSolutionModel(
            name = root.get("name").asString,
            akashaVersion = root.get("akasha_version").asString,
