...
/**
 * Hand-written lexer for Akasha. v0.1 scope: produces enough token
 ...
 *
 * This is intentionally not a full parser — Akasha's real grammar lives
 * in `sdk/akasha-compiler/akasha_compiler/parser.py` and is reached via
 * [AkashaExternalAnnotator]. A future Kotlin port (or a JFlex-based
 * regeneration from the Python reference) will replace this.
 *
 * Keep the keyword / primitive / builtin sets in sync with:
 *   - `sdk/akasha-compiler/akasha_compiler/lexer.py`
 *   - `sdk/vscode-akasha/syntaxes/akasha.tmLanguage.json`
 *   - `sdk/vscode-akasha/src/extension.ts`
 *   - `planning/akasha-spec/21-builtins.md`
 */
class AkashaLexer : LexerBase() {
    ...
    override fun advance() {
        ...
        if (c == '/' && tokenStart + 1 < bufferEnd) {
            ...
            if (next == '/') {
                ...
                tokenEnd = i
                tokenType = if (isDoc) AkashaTokenTypes.DOC_COMMENT else AkashaTokenTypes.LINE_COMMENT
                return
            }
            if (next == '*') {
                var i = tokenStart + 2
                ...
                tokenType = AkashaTokenTypes.BLOCK_COMMENT
                return
            }
        }
        ...
        if (c == '#') {
            ...
            while (i < bufferEnd && buffer[i] != '\n') i++
            tokenEnd = i
            tokenType = AkashaTokenTypes.HASH_COMMENT
            return
        }
        ...
        if (c == '$' && tokenStart + 1 < bufferEnd && buffer[tokenStart + 1] == '"') {
            ...
            tokenType = AkashaTokenTypes.INTERP_STRING
            return
        }
        ...
        if (c == '"') {
            ...
            tokenType = AkashaTokenTypes.STRING
            return
        }
        ...
        if (c.isDigit()) {
            ...
            tokenEnd = i
            tokenType = AkashaTokenTypes.NUMBER
            return
        }
        ...
        if (c == '|' && tokenStart + 1 < bufferEnd && buffer[tokenStart + 1] == '>') {
            ...
            tokenType = AkashaTokenTypes.PIPE_FORWARD
            return
        }
        ...
        if (tokenStart + 1 < bufferEnd) {
            ...
            if (pair in TWO_CHAR_OPERATORS) {
                tokenEnd = tokenStart + 2
                tokenType = AkashaTokenTypes.OPERATOR
                return
            }
        }
        ...
        tokenType = when (c) {
            ...
            in SINGLE_CHAR_OPERATORS -> AkashaTokenTypes.OPERATOR
    ...
    private fun classifyWord(word: String): IElementType = when {
        word in KEYWORDS -> AkashaTokenTypes.KEYWORD
        word in PRIMITIVE_TYPES -> AkashaTokenTypes.PRIMITIVE_TYPE
        word in BUILTINS -> AkashaTokenTypes.BUILTIN
        word in BOOLEAN_LITERALS -> AkashaTokenTypes.BOOLEAN_LITERAL
        word.isNotEmpty() && word[0].isUpperCase() -> AkashaTokenTypes.TYPE_NAME
        else -> AkashaTokenTypes.IDENTIFIER
    }
    ...
    companion object {
        ...
        private val BUILTINS: Set<String> = setOf(
            ...
            // VSA builtin signatures from planning/akasha-spec/21-builtins.md
