 ...
 */
class AkashaCompletionContributor : CompletionContributor() {

    init {
        extend(
            CompletionType.BASIC,
            PlatformPatterns.psiElement().withLanguage(AkashaLanguage),
            AkashaWordCompletionProvider(),
        )
    }

    private class AkashaWordCompletionProvider : CompletionProvider<CompletionParameters>() {
            ...
            for ((word, doc) in KEYWORDS) {
                result.addElement(
                    LookupElementBuilder.create(word)
                        ...
                        .withTailText("  — ${doc.ifBlank { "Akasha keyword" }}", true)
                        .withBoldness(true),
                )
            }
            for ((word, doc) in PRIMITIVE_TYPES) {
                result.addElement(
                    LookupElementBuilder.create(word)
                        ...
                        .withTailText("  — ${doc.ifBlank { "Akasha primitive type" }}", true),
                )
            }
            for ((word, doc) in BUILTINS) {
                result.addElement(
                    LookupElementBuilder.create(word)
                        ...
                        .withTailText("  — ${doc.ifBlank { "Akasha built-in function" }}", true),
                )
            }
        }
    }
    ...
    companion object {
        /**
         * Keyword list — mirrors `sdk/vscode-akasha/src/extension.ts`.
        ...
        private val PRIMITIVE_TYPES: List<Pair<String, String>> = listOf(
            ...
            "vector" to "Hypervector in semantic space. The core Akasha primitive.",
        ...
        private val BUILTINS: List<Pair<String, String>> = listOf(
            ...
            // VSA builtins from planning/akasha-spec/21-builtins.md
