...
See [`planning/akasha-spec/20-ide-architecture.md`](../../planning/akasha-spec/20-ide-architecture.md)
...
- **Hand-written lexer** covering all four comment forms (`//`, `/* */`,
  ...
  bracket delimiters. Word lists mirror `sdk/akasha-compiler` and the VS
  Code extension.
- **Syntax highlighter + color settings page** — every token category is
  rebindable under *Settings → Editor → Color Scheme → Akasha*.
...
- **Live templates** — every snippet from `sdk/vscode-akasha/snippets/akasha.json`
...
```bash
cd sdk/intellij-akasha
gradle wrapper           # first time only
./gradlew runIde         # launches a sandbox IntelliJ IDEA Community
./gradlew buildPlugin    # produces build/distributions/Akasha-0.1.0.zip
```

The sandbox instance opens with the Akasha plugin installed; drop any
`.ak` file into it (e.g. `akasha-demo-program.ak` from the repo root) to
...
```
python -m akasha_compiler --json <file>.ak
```
...
sdk/intellij-akasha/
  ...
  src/main/
    kotlin/org/akasha/intellij/
      AkashaLanguage.kt              Language singleton
      AkashaFileType.kt              .ak file type + icon
      AkashaIcons.kt
      AkashaTokenType.kt             Base IElementType class
      AkashaTokenTypes.kt            Catalog of every leaf token
      AkashaLexer.kt                 Hand-written LexerBase subclass
      AkashaFile.kt                  PsiFile for .ak
      AkashaParserDefinition.kt      Flat parser (no real grammar yet)
      AkashaBraceMatcher.kt          {}, (), [] pairing
      AkashaCommenter.kt             Ctrl+/ toggles //
      AkashaQuoteHandler.kt          Quote auto-insertion for "..." and $"..."
      AkashaSyntaxHighlighter.kt     Lexer-driven highlight categories
      AkashaSyntaxHighlighterFactory.kt
      AkashaColorSettingsPage.kt     Settings UI entry
...
1. `sdk/akasha-compiler/akasha_compiler/lexer.py` — the reference
   lexer, which the external annotator shells out to.
2. `sdk/vscode-akasha/syntaxes/akasha.tmLanguage.json` and
   `sdk/vscode-akasha/src/extension.ts` — the VS Code extension.
3. `sdk/intellij-akasha/src/main/kotlin/org/akasha/intellij/AkashaLexer.kt`
   and `AkashaCompletionContributor.kt` — this plugin.

If you add a keyword, a primitive type, or a builtin to the language,
update all three, plus `planning/akasha-spec/21-builtins.md` if it's a
VSA builtin.
