...
def _builtin_is_string_truth(args: List[str]) -> str:
    # Tag type-test as a fuzzy truth value (NOT the host-bool `is_string`):
    # +1 on AXIS_TRUTH iff AXIS_STRING_FLAG is set, else -1. Substrate-pure
    # scatter, so it composes in the `defuzzy` guard blend (the Elixir/Erlang
    # `is_binary`/`is_bitstring` guards lower here).
