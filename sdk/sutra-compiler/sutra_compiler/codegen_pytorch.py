...
class PyTorchCodegen(Codegen):
    ...
    def _emit_prelude(self) -> None:
        ...
        self._emit("av = self._as_any_vector(a)")
        self._emit("bv = self._as_any_vector(b)")
        self._emit("# Cosine-fuzzy equality on the substrate: similarity")
        self._emit("# (∈ [-1, 1] for unit-norm vectors, ~+1 for identical).")
        self._emit("# Lift it onto the truth axis, then defuzzify_trit to")
        self._emit("# polarize. Result is a substrate vector with AXIS_TRUTH")
        self._emit("# saturated toward +1 (strictly equal) or -1 (strictly")
        self._emit("# unequal).")
        self._emit("sim = self.similarity(av, bv)")
        self._emit("fuzzy_eq = self.make_truth(sim)")
        self._emit("return self.defuzzify_trit(fuzzy_eq)")
        self._indent -= 1
        self._emit()
