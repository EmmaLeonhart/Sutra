...
class CodegenBase:
    ...
    Substrate-neutral scaffolding: AST walk, expression translation,
    builtin dispatch, loop shape emission. Each backend subclass
    (FlyBrainCodegen, NumpyCodegen, future TorchCodegen) supplies its
    own `_emit_prelude` that defines the `_VSA` runtime, and its own
    `_translate_eigenrotation_loop` with substrate-appropriate
    rotation parameters.
    ...
    def __init__(self, *, runtime_dim: int = 50, runtime_seed: int = 42) -> None:
    ...
    def _emit_prelude(self) -> None:
        # Substrate-specific: each backend writes its own _VSA
        # runtime scaffolding. Subclasses override.
        raise NotImplementedError(
            "CodegenBase does not supply a prelude; each backend subclass "
            "must override _emit_prelude with its substrate-specific runtime"
        )
    ...
    def _translate_eigenrotation_loop(self, stmt: ast.LoopStmt) -> None:
        # Substrate-specific: each backend picks its own rotation
        # magnitude and plane count. Subclasses override.
        raise NotImplementedError(
            "CodegenBase does not supply an eigenrotation-loop lowering; "
            "each backend subclass must override _translate_eigenrotation_loop"
        )

    _loop_counter = 0  # unique names for loop temporaries

    def _next_loop_id(self) -> str:
        CodegenBase._loop_counter += 1
        return f"_loop{CodegenBase._loop_counter}"
...
class FlyBrainCodegen(CodegenBase):
    """Sutra → Brian2 spiking MB + FlyBrainVSA Python emission.

    Everything AST-walking lives in `CodegenBase`. This subclass layers
    on the fly-brain-specific runtime scaffolding (spiking simulator
    import, FixedFrameFlyBrainVSA class, _argmax_cosine /
    _select_softmax / _vector_map_lookup helpers emitted inline) and
    the fly-brain-flavored eigenrotation-loop lowering (pi/4 per-step
    rotation across 20 planes, matching the MB circuit's separation
    characteristics).
    """
