...
/**
 * File type registration for `.aksln` (Sutra Solution) files.
 *
 * These files are TOML documents per `planning/sutra-spec/22-solutions.md`.
 * We register them as a plain-text file type here rather than declaring
 * them as a dedicated Sutra Solution language because:
 *
 ...
 *
 * The file type exists primarily so the Sutra Solution tool window
 ...
 * correct editor view.
 */
class SutraSolutionFileType : LanguageFileType(PlainTextLanguage.INSTANCE) {
    override fun getName(): String = "Sutra Solution"
    override fun getDescription(): String =
        "Sutra solution file (TOML; enumerates the projects in a solution)"
    override fun getDefaultExtension(): String = "aksln"
    override fun getIcon(): Icon? = null  // future: bundle an icon

    companion object {
        @JvmField
        val INSTANCE = SutraSolutionFileType()
    }
}
