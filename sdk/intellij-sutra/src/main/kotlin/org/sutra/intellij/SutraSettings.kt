...
/**
 * Application-level persistent settings for the Sutra plugin.
 *
 ...
 *
 *   1. This settings service (set via Settings → Tools → Sutra)
 *   2. Environment variables SUTRA_COMPILER / SUTRA_COMPILER_ARGS
 *   3. Hardcoded defaults (`python` + `-m sutra_compiler`)
 *
...
@State(
    name = "SutraSettings",
    storages = [Storage("sutra.xml")],
)
class SutraSettings : PersistentStateComponent<SutraSettings.State> {
    ...
    fun effectiveCompiler(): String {
        ...
        val env = System.getenv("SUTRA_COMPILER")?.trim()
    ...
    fun effectiveCompilerArgs(): String {
        ...
        if (configured.isNotEmpty()) return configured
        val env = System.getenv("SUTRA_COMPILER_ARGS")?.trim()
        if (!env.isNullOrEmpty()) return env
    ...
    companion object {
        ...
        const val DEFAULT_COMPILER_ARGS = "-m sutra_compiler"

        /** Convenience accessor: `SutraSettings.getInstance()` from anywhere. */
        @JvmStatic
        fun getInstance(): SutraSettings = service()
    }
}
