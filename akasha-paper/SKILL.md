...
**Source:** `akasha-paper/` (this directory), with runtime scripts under `akasha-paper/scripts/`. The biological-substrate result reported in §6.6 is reproduced by the sibling `fly-brain-paper/` skill; see that paper's SKILL.md for the mushroom-body pipeline.
...
Every script in `akasha-paper/scripts/` is runnable standalone and writes its outputs to `akasha-paper/data/` so the paper's tables can be regenerated deterministically.

**Sign-flip binding vs. alternatives (six binding operations on GTE-large):**
```bash
python akasha-paper/scripts/binding_alternatives_experiment.py
```
Outputs `akasha-paper/data/binding-alternatives-gte-large.json`. Produces the table in §6.2 showing Hadamard failing at 2+ roles while sign-flip, permutation, circular convolution, FFT correlation, and rotation binding all achieve 7/7 recovery at 7 bundled roles.

**Deep sign-flip capacity test (14 roles):**
```bash
python akasha-paper/scripts/signflip_deep_test.py
```
Outputs `akasha-paper/data/signflip-deep-gte-large.json` and `akasha-paper/data/signflip-cross-substrate.json`. Runs sign-flip through 14 bundled pairs, 10-step chained computation, and multi-hop composition across all three tested substrates.

**Chain depth and snap cost analysis:**
```bash
python akasha-paper/scripts/chain_depth_experiment.py
```
Outputs `akasha-paper/data/chain-depth-gte-large.json`. Demonstrates that pure bind–unbind chains without snap accumulate unrecoverable noise, while snap-interleaved chains stabilize.

**Bundling noise characterization:**
```bash
python akasha-paper/scripts/bundling_noise_experiment.py
```
Outputs `akasha-paper/data/bundling-noise-gte-large.json`. Quantifies how signal-to-noise degrades as more items are bundled into a single superposition — the main capacity ceiling in the algebraic tier.

**Cross-substrate empirical initiation gates:**
```bash
python akasha-paper/scripts/empirical_initiation.py
```
...
`akasha-paper/scripts/akasha_runtime.py` is the reference runtime used by the demos. It is a standalone Python implementation of:
...
- `akasha-paper/scripts/akasha_runtime.py` — reference runtime (imported by demos)
- `akasha-paper/scripts/akasha_demos.py` — end-to-end program demos
- `akasha-paper/scripts/binding_alternatives_experiment.py` — §6.2 binding table
- `akasha-paper/scripts/signflip_deep_test.py` — §6.2 14-role + chained + multi-hop
- `akasha-paper/scripts/chain_depth_experiment.py` — chain noise characterization
- `akasha-paper/scripts/bundling_noise_experiment.py` — bundling capacity curves
