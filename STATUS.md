...
3. ~~Surface-syntax re-evaluation~~ **DECIDED 2026-04-21: Candidate B (`role` for semantic, `var` for rotation-bound).** The syntax now looks like `role capital_of = learned_from("cities.tsv");` for semantic bindings and `var x : fuzzy = +0.7;` / `var[16] slots : vector;` for rotation-bound storage and arrays. The resolved open-question doc is retained at `planning/open-questions/binding-kind-surface-syntax.md` pending its move into `planning/sutra-spec/binding.md` as the Surface syntax section. Follow-on work: (a) add Surface-syntax section to `binding.md` with examples of both kinds, (b) update the three demo programs (`hello_world`, `fuzzy_branching`, `role_filler_record`) from the current `vector r_name = basis_vector(...)` pattern to `var r_name : vector`, (c) teach `sdk/sutra-compiler/` the new declaration forms.
