...
- **Compiler pipeline** — `sdk/sutra-compiler/` emits Python that calls `fly-brain/vsa_operations.py`. `.su` programs compile through it: `fly-brain/fuzzy_conditional.su`, `fly-brain/three_way_conditional.su`, `fly-brain/bind_unbind_chain.su`, `fly-brain/geometric_loop.su`.
...
- **Rotation from real wiring** 🟢 **COMPOSED ACROSS 4 MOTIFS, 713-D; SPIKING LOOP SIM_MS-SCALED** — surveyed 11 FlyWire motifs (`fly-brain/survey_rotation_candidates.py`). Winner: **CX EPG→EPG recurrent, 51 neurons, effective rank 49, off-diagonal fraction 0.508** — an order of magnitude closer to orthogonal than ALPN→LHLN. Polar decomposition (`fly-brain/real_rotation_epg.py`) + Kabsch sign correction yields Q = nearest orthogonal matrix to W with QᵀQ = I to 1e-14, det Q = +1. Numpy-only geometric loop passes 10/10 counting + 5/5 ordering up through composed 713-D Q (`real_rotation_composed.py`). Pipeline A (substrate-only spiking loop, 140-D block-diag Q from EPG-51 ⊕ hDelta-89): 9/10 at k=3 (n=5 seeds) and a 14/30 k-sweep at SIM_MS=3000ms; extending the Brian2 integration window lifts the k-ceiling — 6000 ms gives 3/3 at k=3, 3/3 at k=5, 1/3 at k=8; 12000 ms pushes k=8 to 2/3. The ceiling is not structural. Pipeline B (numpy rotation + MB-Jaccard readout on real hemibrain): 20/20 + 30/30. Combined (spiking rotation → MB-Jaccard) 0/5 — MB is an anti-correlator; cannot ingest Poisson-noised rotation output. Honest framing: "rotation operator within the subspace spanned by the EPG/hDelta recurrent projection, derived via polar decomposition from real FlyWire weights." Not "the biology IS the rotation" — but not synthetic Givens either.
