...
1. **🔴 META-BLOCKER: Fly-brain Python-file sprawl cleanup.** User direction (2026-04-13): *"The fly-brain clutter is the biggest issue; it is the biggest limiting factor here ... it loads up context with bullshit. It's probably the reason why things are so slow."* Every other queue item below pays a context-bloat tax until this is done. **Must be done locally** — Claude Code in the sandbox cannot distinguish "still passes the spiking sim" from "merely imported," because the sandbox has neither Brian2 nor the 14 GB FlyWire cache. The actionable set, from the `todo.md` audit:
