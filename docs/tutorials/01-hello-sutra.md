...
You wrote source code that the compiler parsed, validated against the grammar, type-checked (such as the v0.1 type checker can — see the [Operations and operators](../operators.md) reference for what it actually verifies), and accepted. You did **not** run it on a real embedding model yet — that requires the Sutra runtime, which you'll meet in tutorial 03 when we hook up `snap-to-nearest`.
