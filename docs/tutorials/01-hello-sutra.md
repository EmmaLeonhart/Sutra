...
You wrote source code that the compiler parsed, validated against the grammar, type-checked (such as the v0.1 type checker can — see the [language spec](https://github.com/EmmaLeonhart/Sutralang/tree/master/planning/sutra-spec) for what it actually verifies), and accepted. You did **not** run it on a real embedding model yet — that requires the Sutra runtime, which you'll meet in tutorial 03 when we hook up `snap-to-nearest`.
...
- The [language spec on operations](https://github.com/EmmaLeonhart/Sutralang/blob/master/planning/sutra-spec/02-operations.md) — the formal definition of the three operation tiers.
