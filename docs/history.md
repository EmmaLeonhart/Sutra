...
The project was originally called **Ākaśa** (Sanskrit: "sky," "space," "aether") — evoking the concept of a primordial medium through which all things exist and connect, which is how a pre-trained embedding space feels: an ambient semantic field that pervades everything. Ākaśa was renamed to Sutra on 2026-04-11. The earlier name still appears across older commits. The `.ak` file extension (now `.su`) and workspace format (`atman.toml`, formerly `.aksln`/`.akproj`) are traces of this history.
...
Natural LLM embeddings are not the random orthogonal vectors that VSA theory assumes. The textbook Hadamard binding fails completely on them — correlated, anisotropic geometry breaks the unbinding algebra. Sutra's bind operation is rotation binding: a role-seeded Haar-random orthogonal matrix `Q_role`, with `bind(role, filler) = Q_role @ filler` and `unbind(role, record) = Q_role^T @ record`. This recovers correct fillers from bundled role-filler structures across the substrates Sutra targets (`nomic-embed-text`, `mxbai-embed-large`, `all-minilm`). The longer-term direction is *learned* role matrices fit to corpus data — the rank-0 case is the displacement-vector cartography work, the full-rank case is Sutra's "semantic binding" (deferred implementation).
...
The Sutra fly-brain experiment ([paper](https://clawrxiv.io/posts/1541)) compiled four-state conditional programs onto a Brian2 spiking simulation of this circuit and got 16/16 correct decisions. A follow-on phase tested rotation, snap, and conditional branching against the Shiu et al. 2024 whole-brain LIF model, producing mostly negative findings (real FlyWire weight matrices do not function as rotation operators; CX ring-attractor circuits did not discriminate direction on real connectivity). The supporting fly-brain code was retired from the repo on 2026-04-26 — the substrate work outpaced the language's maturity. The research line may resume once the language is more mature.
