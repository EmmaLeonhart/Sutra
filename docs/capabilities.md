...
| `(Type) value` (C-style cast) | conversion cast — no-op relabel by default; numeric↔truth casts axis-move (`(fuzzy) 0.7` puts 0.7 on the truth axis; `(number) b` reads a truth value back as **signed ±1** — `true` → `+1`, `false` → `−1`, `unknown` → `0`, matching the Kleene truth scale; a 0/1 counter therefore maps via `((number) cond + 1) / 2`) via cached matmuls; `(string) n` on an `int` formats via `int_to_string`; other text casts are rejected at codegen (fractional rendering is not built; string→vector is `embed()`'s job). Guard SUT0111 still steers `(vector) "literal"` to `embed(...)` | n/a |
...
| `realvec(v)` | project a vector to a CLEAN real-axis number-vector (keep the real axis, zero the rest) via the real-axis projector matmul — stays a tensor, no host readout; the in-language replacement for the removed `real()` accessor, e.g. to decode an axon field filler that carries crosstalk in non-real dims | n/a |
| `is_string_truth(v)` / `is_number_truth(v)` | type-test as fuzzy truth: +1 on the truth axis if `v` carries the String flag (resp. does NOT carry it), else −1 — substrate-pure scatter. Caveat: `is_number_truth` cannot distinguish a number from an axon (axons carry no clean runtime tag) | n/a |
| `sin_buf(x)` / `cos_buf(x)` | elementwise sin/cos over a length-N field buffer (the buffer counterparts to scalar `sin`/`cos`): wrap to (−π,π], then a triangular soft-index crosstalk readout of the cached sin/cos table — substrate-pure, periodic, autograd-preserving, length-N in/out | n/a |
| `geometric_loop(state, rotation, prototypes[, target_name])` | the low-level lowering target of `loop (condition)`: iterates `state ← R·state` on the substrate with the match evaluated against compiled prototypes — user code normally writes `loop` forms (docs/loops.md), not this directly | n/a |
| `ramRead(ptr)` / `ramWrite(ptr, data)` | RAM-pointer I/O: boundary I/O, not a substrate op — the pointer is substrate-computed, the value crosses at a loop/program boundary through the promise machinery (`await ramRead(ptr)`); see [the I/O model](host-bridge.md) | n/a |
