...
4. **Cross-substrate empirical initiation gates.** A formal calibration phase tests an embedding model's algebraic fitness, fits correction matrices, and either approves or rejects it for Akasha use. Sign-flip improves effective capacity 3–5× over Hadamard across all four tested substrates. The mxbai-embed-large pathology (diacritic attention sink) is caught by *pathology-specific* probes that run alongside the algebraic gates — substrates can pass algebraic tests and still have silent corruption modes.
...
**The result:** an Akasha source file describing a four-state conditional is parsed and validated by the same compiler used for the silicon experiments (paper 1), translated by a substrate-specific backend (`sdk/akasha-compiler/akasha_compiler/codegen_flybrain.py`) into Python calls against the spiking circuit, and executed. Across four program variants × four input conditions (sixteen decisions total), the compiled output produces the expected behavior mapping on every trial. The four variants yield four distinct permutations of the underlying behavior table.

To our knowledge, this is **the first demonstration of a programming language whose conditional semantics compile mechanically onto a connectome-derived spiking substrate**. It serves as a non-silicon stress test for the substrate-agnostic claim that Akasha is supposed to satisfy.
...
- **Loops are intentionally unsupported.** A `while` compilation path probably needs recurrent KC→KC connections that the current circuit doesn't have. Framed as a research question, not a codegen oversight. See [`fly-brain/STATUS.md`](https://github.com/EmmaLeonhart/Akasha/blob/master/fly-brain/STATUS.md) §Loops.

## How these papers get to clawRxiv

Both papers are published to [clawRxiv](https://clawrxiv.io) automatically by [`.github/workflows/papers-ci.yml`](https://github.com/EmmaLeonhart/Akasha/blob/master/.github/workflows/papers-ci.yml) on every push to `master` that touches their `paper.md` or `SKILL.md`. The workflow:
...
The submission logic lives in [`scripts/paper_submit_and_fetch.py`](https://github.com/EmmaLeonhart/Akasha/blob/master/scripts/paper_submit_and_fetch.py) so it can be run locally for one-off backfills, and so the workflow YAML stays small.
