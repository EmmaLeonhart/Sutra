"""
S2 Demo Programs — End-to-End Execution on Embedding Substrates
================================================================

These demos show S2 executing real programs on real embedding spaces.
Each demo is a self-contained S2 program that demonstrates a different
capability of the language.

Usage:
    python S2-paper/scripts/s2_demos.py
    python S2-paper/scripts/s2_demos.py --model gte-large
    python S2-paper/scripts/s2_demos.py --demo associative_memory
    python S2-paper/scripts/s2_demos.py --all-demos
"""
...
def demo_associative_memory(env):
    """
    S2 program: build an associative memory, store role-filler pairs,
...
def demo_multi_hop(env):
    """
    S2 program: multi-hop reasoning via chained bind/unbind.
    ...
    # In a full S2 program, we'd have a fact store; here we query fact2 directly
...
def demo_composition(env):
    """
    S2 program: compositional structure manipulation.
...
def demo_cone_traversal(env):
    """
    S2 program: navigate embedding space using cone traversal.

    Start at a concept, define a direction, find what's "in that direction"
    in semantic space. This is S2's non-algebraic control flow.
    """
...
def demo_structured_matching(env):
    """
    S2 program: structured matching with confounder control.
...
def demo_truth_extraction(env):
    """
    S2 program: fuzzy truth evaluation with recursive sharpening.
...
def main():
    parser = argparse.ArgumentParser(description="S2 Demo Programs")
    ...
    print(f"S2 Runtime — Substrate: {substrate.model_name}")
    print(f"{'='*60}")
