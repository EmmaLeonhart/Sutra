...
(`demos/**/*.su` + `external/Yantra/**/*.su`) plus four `.py`-with-inline-`.su` the earlier sweeps missed
(the `generate_font_bound{,_antipodal}_su.py` generator templates, `experiments/bench_tick_all.py`,
`external/Yantra/tests/test_kernel_sutra.py`). Swept to ZERO `basis_vector(` call sites, then DELETED
`_builtin_basis_vector` + its BUILTINS entry. Cleaned the now-dead `_is_basis_vector_literal_call` branch
and renamed `collect_basis_vector_strings` → `collect_embedded_strings` (it keys off `EmbedExpr` only now;
updated all call sites in `codegen.py`/`codegen_pytorch.py`/`dimension_audit_sweep.py`/`test_simplify`/
`test_dimension_audit`). `embed` is the single canonical spelling. Fast subset green (178 passed); FULL
suite + `pytest demos/` (font cold ~25 min) + smoke running to prove behaviour-identical before commit.

## 2026-06-23: alias batch (3) — `scalar` type + `iff` keyword FULLY REMOVED

Emma corrected the scalar call: the paper + replication package are NO LONGER FROZEN, so the
backward-compat reason for keeping `scalar` is void — drop it entirely. And `iff` → `xnor` after all
(remove iff), though the OTHER logical-connective spellings stay (her call, she's unsure).

**`scalar` removed:** repointed ~1300 type-position sites `scalar`→`number` via a targeted regex (the
hazard was the substrate-purity PROSE — "scalar extraction", "host scalar", "make_real(scalar)" — which
the patterns deliberately don't match; verified prose untouched). Scope: all `.su` (examples, corpus,
demos incl. font.su's 980 sites, Yantra) + `tests/*.py` inline strings + the font generators
(`generate_font*.py` templates) + `paper/supplementary/SYNTAX.md` + a few experiment/demo `.py`. Removed
`scalar` from `PRIMITIVE_TYPE_NAMES` (lexer), `_PRIMITIVE_TYPES` (parser), the validator `PRIMITIVES` set,
and codegen's `_NUMBER_TYPES`/`_SYNTHETIC_AXIS_TYPES`/init-classifier sets. Deleted the now-obsolete
SUT0114 deprecation path + `test_scalar_deprecation.py` + the `TestNumberScalarAlias` class. Fixed a
test asserting the old type name (`test_const_typed`) and the `iff` codegen tests.

**`iff` removed:** repointed `examples/logical_connectives.su` to `xnor`, dropped the `iff` lexer entry +
the `test_iff_alias_for_xnor` test (the identifier test repointed to `Xnor`). CLAUDE.md records both
decisions + the carve-out keeping other connective spellings.

Fast subset green (191 passed); FULL suite + `pytest demos/` (font ~400s) + smoke running to prove the
~1300-site repoint is behaviour-identical before the commit lands.
...
`tests/corpus/valid/29_unknown_literal.su` to `unknown`, removed the `"unk": KW_UNKNOWN` lexer entry +
its comment. Verified — `unk` now lexes as IDENT, lexer imports, corpus 3 passed / 90 subtests.
`iff` → ⚠ left for Emma: it aliases `xnor` but is standard math notation AND is actively demonstrated in
`examples/logical_connectives.su` (`iff_keyword`), so retiring it removes a documented feature — flagged
in queue.md as her call (keep as the one intentional exception, or retire). Not touched.
...
to an `EmbedExpr` and `collect_basis_vector_strings` collects it identically to `basis_vector` (verified —
so the batched pre-fetch is preserved); the stdlib "definitions" are commented-out docs, not real
functions; the axon runtime calls `_VSA.embed` directly (surface alias irrelevant). The one real internal
dependency was thrml's `_basis_atoms`, which only recognised `Call(basis_vector)` — taught it to also
accept `EmbedExpr` (`embed`). Then repointed all `examples/*.su` (233) + `docs/` (20) to `embed`, fixed
the measurably-false "random basis / pairwise cosine ~0 / concentration of measure" comments in
`nearest_phrase.su` + `classifier.su` (real embeddings, ~0.47 correlated), and marked
`docs/operators.md`'s `basis_vector` entry deprecated. Behaviour-identical: **smoke test PASS** (12/12
examples, identical outputs), **thrml 2 passed**, site builds. `_builtin_basis_vector` kept as a
DEPRECATED thin alias (`# DEPRECATED 2026-06-23`) until the remaining corpus + inline-test-string sites
(~181) and the demos/font + Yantra subtree migrate — then it's deleted. Queue item updated with the
remainder.
...
dispatch in `codegen_base.py` — `BUILTINS` → else `intrinsic_names()` → `_VSA.<name>` → else user call.
`make_real`/`make_truth`/`make_complex`/`embed` are all intrinsics (`stdlib_loader.intrinsic_names()`),
so they resolve via the `_VSA` passthrough independent of `BUILTINS`; deleting a `*_number`/`*_value`/
`basis_vector` alias leaves the canonical form reachable. **Item 1:** removed the `truth_value` +
`complex_number` BUILTINS entries + `_builtin_*` defs (pure aliases). The audit's "0 sites" was `.su`-only
— there WERE inline test-string call sites (`test_transcendentals.py` constructed complex numbers via
`complex_number`; `test_codegen.py` had alias-lowering tests). Repointed all to the canonical
`make_complex`/`make_truth` (still lower to `_VSA.make_*`, coverage preserved — not weakened). 99
passed in test_codegen + test_transcendentals. `real_number` kept (item 2, has 6 demo/Yantra sites).

## 2026-06-23: usability loop (batch 2) — tutorial 04 Ollama fix + stale-count sweep + onboarding polish

Three audit items in one doc-accuracy commit. (1) `docs/tutorials/04-from-typescript.md` was the lone page
still saying programs embedding text "need a local Ollama server" — aligned it to the in-process `[embed]`
story (consistent with every other page). (2) The smoke test runs 12 examples now (added `semantic_faq`);
refreshed the stale counts/listings: `README.md` (11→12 + new row), `docs/demos.md` (Ten→Twelve + the
missing `content_addressed_read` 7b AND `semantic_faq` 10 rows + "smoke-tested ten" prose + dropped a
year-old "new loop surface"), `docs/what-is-sutra.md` (ten→twelve), `scripts/build_site.py` homepage
Tutorials blurb (now names TypeScript + the FAQ matcher). (3) Onboarding polish: README "Get started" now
states Python 3.11+; tutorials index cross-links `list-operations.md`. Site builds clean; all pages render.
...
(intercepted before argparse, which requires a `paths` positional). Type an expression → it wraps it in
`function vector __repl_eval__()`, compiles + runs the whole session on the PyTorch substrate, and decodes
the FINAL result at the terminal boundary — the same sanctioned read `sutrac --run` already does
(`_decode_terminal_result`): a number-vector shows its real-axis value; any other vector decodes to the
nearest known concept by cosine argmax over `_VSA._codebook` (the demos' `argmax_cosine`-over-codebook
pattern), shown `≈ "concept" (cos 0.NN)`. NO `.real()`/`.item()` inside any operation — readout-free per
CLAUDE.md. Lines ending `;`/`}` accumulate as session declarations (validated before keeping, so a bad
line can't poison later evals; multi-line decls read until braces balance); `:help`/`:decls`/`:reset`/
`:quit` meta-commands; errors are caught and the loop continues. Verified live (`6*7` → `= 42`,
`embed("king")` → `≈ "king" (cos 1.00)`). Tests `tests/test_repl.py` (6, model-free via `make_real`).
README CLI note added.
...
confirmed: METADATA carries `Provides-Extra: embed` with `sentence-transformers>=3.0` + `einops>=0.7`
(and `runtime` → torch); `sutra_compiler/embedding.py` ships in the wheel; the `sutrac` console entry
point is wired; 17 stdlib `.su` files included. Clean-venv install of the base wheel (numpy only) imports
`sutra_compiler` + `sutra_compiler.embedding` and runs `sutrac --help`. `pip install --dry-run
".whl[runtime,embed]"` resolves the full stack (torch, sentence-transformers, einops, transformers,
huggingface-hub, scikit-learn) with no conflict. So a PyPI `pip install "sutra-dev[runtime,embed]"`
is sound.
...
correctness/substrate-purity). Picked the highest-reach barrier: every emitted program did
`import ollama; ollama.embed(...)` with no fallback, so `pip install sutra-dev` could not run a single
semantic program until the user also installed Ollama + pulled a model. Emma's call: `nomic-embed-text`
is a *frozen* off-the-shelf model we never customize, so load it in-process and embed directly.

Built `sutra_compiler/embedding.py` — `embed_texts(names, model)` with `SUTRA_EMBED_BACKEND`
(auto = in-process-first → Ollama fallback | transformers | ollama). Both codegens now call the provider
instead of inlining Ollama. On-disk cache is now backend-aware (`{model}-d{dim}-{backend}.{pt,npz}`) so
the two backends — different geometry for the same model — don't read each other's vectors. New `embed`
extra in `pyproject.toml` (`sentence-transformers`, `einops`); README + homepage onboarding rewritten to
the daemon-free `pip install "sutra-dev[runtime,embed]"` path.

**Measured, not assumed** (finding: `planning/findings/2026-06-22-in-process-embeddings-drop-ollama-daemon.md`):
in-process `nomic-embed-text-v1.5` vs Ollama's GGUF nomic = mean cosine 0.88 — two realizations of the
same model family, not identical vectors. The example smoke test PASSES end-to-end on the in-process
substrate with no Ollama. One compiler stress test (`test_axon_op_cache_under_cap_never_evicts`) collides
under the in-process geometry (short word "go" near "telephone"), so the **test suite is pinned to its
tuned substrate, Ollama** (`tests/conftest.py`) while the **user default is in-process**. Re-tuning the
whole suite to the in-process geometry is left as separate work. Full compiler suite green under Ollama;
new `tests/test_embedding_provider.py` covers the dispatch/mapping + both backend paths.
...
## 2026-06-19 — daily audit: clean (end-to-end, including semantic await tests)

73 .su compiled, 18 skipped, 0 user-program leaks + 0 runtime-prelude leaks (`experiments/substrate_leak_sweep.py`); `scripts/check_promise_await_fit_to_spec.py` returns `EXIT=0` with `[2/2] regression tests  PASS (4/4 expected)`; `tests/test_no_host_readout.py` 5/5 + `tests/test_substrate_leak_sweep.py` 1/1 PASS (6 total in 61s). Same shape as 06-15/06-16: after installing `pytest`, `numpy`, `torch` (CPU), the `ollama` Python pkg, and the Ollama daemon binary (`apt-get install zstd && bash /tmp/ollama_install.sh` from `https://ollama.com/install.sh`, then `ollama serve` + `ollama pull nomic-embed-text`), the 2 semantic-preservation tests (`test_await_semantics_preserved_{torch,numpy}`) DID run and PASS to 3 places against the fixture's expected 3.0; end-to-end promise/await verification is CLEAN, not partial. Compiled count unchanged 73 → 73 since 06-16; the only commit touching `codegen_pytorch.py` since the 06-16 audit is `3c59427` (queue §0.3: compound &&/|| AND equality loop halts) — adds `_VSA.truth_and/truth_or/truth_not` runtime methods + makes `_as_any_vector` scatter a 0-d tensor onto the real axis on-device (no `float()`) and `truth_axis` 0-d-safe; substrate-pure by the commit's own contract, verified by the 0-leak sweep. `await_value` body at `codegen_pytorch.py:935–966` is still the spec-compliant `return self.value(p)` reduction per Audit REAL LEAK #3 (FIXED 2026-05-17) — no `for _ in range(100)` / `if self.isPending` signature anywhere; the prior body is referenced only in the leak-history docstring (lines 952–963, phrased without the literal old signature so the sweep doesn't false-positive). Audit.md REAL LEAK #1–#10 all still FIXED/NOT-A-LEAK at cited codegen sites: #9 (`eq`/`eq_synthetic` scatter) intact at `codegen_pytorch.py:2911` / `:2936` (0-d tensor scatter `out[self.semantic_dim + self.AXIS_TRUTH] = cos` / `= truth`, autograd preserved); #10 (`_select_softmax` scores) intact at `codegen_pytorch.py:74` (`_torch.stack([sc.to(...) for sc in scores])` grad-preserving stack with the raw-number `as_tensor` fallback at `:78`); #4 (`_TorchVSA.loop` fixed-T unroll) reconfirmed at `codegen_pytorch.py:3069` (`for _t in range(max_iters)` is structural-index unroll, body is `self._step(state, rotation, target, halted, k, threshold)` — tensors in/out, soft halt via sigmoid, no `.item()`/`float()`/host branch on data). Spot-checked every `_emit(...float(...))` / `_emit(...item()...)` / `_emit(...for ... in range(...))` hit against Audit.md taxonomy + `experiments/substrate_leak_sweep.py::_PRELUDE_LEAK_EXEMPT_METHODS` — every site is documented BORDERLINE (Promise inspectors, `make_real`/`make_truth`/`make_char` literal entry, `array_from_literal`, `load_matrix` CSV text parse, JS-interop equality/promotion + `_js_str_cmp` per CLAUDE.md carve-out, `_argmax_cosine`/`_vector_map_lookup` terminal index, `string_to_python` substrate→host terminal decode, `defuzzify_trit` structural-iters unroll per Audit #4, `digit_array_add` structural-N loop per Audit #4, `ram_read`/`ram_write` I/O-boundary pointer decode per `_PRELUDE_LEAK_EXEMPT_METHODS` carve-out) or LEGITIMATE (compile-time constants PI/TAU); no new sites since 2026-06-16. 16 open-question dossiers in `planning/open-questions/` + `sutra-spec/open-questions.md` cross-checked: README verdict table unchanged from 2026-05-28 pruning; `axon-string-filler-roundtrip.md` still marked RESOLVED 2026-06-08 inline and authoritatively resolved at `planning/sutra-spec/axons.md:285–294` (kept as record per the doc's own note, not "resolved-elsewhere drift" in the audit sense); `2026-06-13-sutra-to-thrml-mapping.md` is an active exploration loop and correctly outside the resolved/stale axis; `paren-cast-vs-grouping-ambiguity.md` still genuinely open per the README table; spec-index strikethroughs (`types §"scalars as results"`, `control-flow §"loop unroll for"`, `binding §"surface syntax"`, `RAM-pointers §"Differentiable / soft addressing"`) unchanged. No code regression detected in any scope.

## 2026-06-16 — daily audit: clean (end-to-end, including semantic await tests)

73 .su compiled, 18 skipped, 0 user-program leaks + 0 runtime-prelude leaks (`experiments/substrate_leak_sweep.py`); `scripts/check_promise_await_fit_to_spec.py` returns `EXIT=0` with `[2/2] regression tests  PASS (4/4 expected)`; `tests/test_no_host_readout.py` 5/5 + `tests/test_substrate_leak_sweep.py` 1/1 PASS (6 total in 63s). Same shape as 06-13/06-14/06-15: after installing `pytest`, `numpy`, `torch` (CPU), the `ollama` Python pkg, and the Ollama daemon binary (`apt-get install zstd && bash <(curl -fsSL https://ollama.com/install.sh)` then `ollama serve` + `ollama pull nomic-embed-text`), the 2 semantic-preservation tests (`test_await_semantics_preserved_{torch,numpy}`) DID run and PASS to 3 places against the fixture's expected 3.0; end-to-end promise/await verification is CLEAN, not partial. Compiled count unchanged 73 → 73 since 06-15; the only commit touching `codegen_pytorch.py` since yesterday's audit is `1be5b17` (F# typed params — adds a Sutra-type mapper for F# typed_pattern params, no substrate-touch). `await_value` body at `codegen_pytorch.py:935–966` is still the spec-compliant `return self.value(p)` reduction per Audit REAL LEAK #3 (FIXED 2026-05-17) — no `for _ in range(100)` / `if self.isPending` signature anywhere; the prior body is referenced only in the leak-history docstring (lines 952–963, phrased without the literal old signature so the sweep doesn't false-positive). Audit.md REAL LEAK #1–#10 all still FIXED/NOT-A-LEAK at cited codegen sites: #9 (`eq`/`eq_synthetic` scatter) intact at `codegen_pytorch.py:2843` / `:2865` (0-d tensor scatter, autograd preserved); #10 (`_select_softmax` scores) intact at `codegen_pytorch.py:74` (`_torch.stack([sc.to(...) for sc in scores])` grad-preserving stack with the raw-number `as_tensor` fallback at `:78`); #4 (`_TorchVSA.loop` fixed-T unroll) reconfirmed at `codegen_pytorch.py:2988` (`for _t in range(max_iters)` is structural-index unroll, body is `self._step(state, rotation, target, halted, k, threshold)` — tensors in/out, soft halt via sigmoid, no `.item()`/`float()`/host branch on data). Spot-checked every `_emit(...float(...))` / `_emit(...item()...)` / `_emit(...for ... in range(...))` hit against Audit.md taxonomy + `experiments/substrate_leak_sweep.py::_PRELUDE_LEAK_EXEMPT_METHODS` — every site is documented BORDERLINE (Promise inspectors, `make_real`/`make_truth`/`make_char` literal entry, `array_from_literal`, `load_matrix` CSV text parse, JS-interop equality/promotion + `_js_str_cmp` per CLAUDE.md carve-out, `_argmax_cosine`/`_vector_map_lookup` terminal index, `string_to_python` substrate→host terminal decode, `defuzzify_trit` structural-iters unroll per Audit #4, `digit_array_add` structural-N loop per Audit #4, `ram_read`/`ram_write` I/O-boundary pointer decode per `_PRELUDE_LEAK_EXEMPT_METHODS` carve-out) or LEGITIMATE (compile-time constants PI/TAU at `:324–325`); no new sites since 2026-06-15. 16 open-question dossiers in `planning/open-questions/` + `sutra-spec/open-questions.md` cross-checked (corrected miscount; yesterday's "15" was off by one — 16 files have existed since `1be5b17`): README verdict table unchanged from 2026-05-28 pruning; `axon-string-filler-roundtrip.md` still marked RESOLVED 2026-06-08 inline (kept as record, not "resolved-elsewhere drift" in the audit sense); `2026-06-13-sutra-to-thrml-mapping.md` is an active exploration loop and correctly outside the resolved/stale axis; `paren-cast-vs-grouping-ambiguity.md` (added 2026-06-04) still genuinely open per the README table; spec-index strikethroughs (`types §"scalars as results"`, `control-flow §"loop unroll for"`, `binding §"surface syntax"`, `RAM-pointers §"Differentiable / soft addressing"`) unchanged. No code regression detected in any scope.
...
## 2026-06-15: Clojure frontend — `(get m :k)` access + string-key maps (Phase 3)

Phase 3 transpiler-backlog increment, extending Clojure maps→axons. (1) The function-call
accessor `(get m :k)` / `(get m "k")` now lowers the same as the `(:k m)` keyword accessor
(`realvec(m.item("k"))`) — intercepted in the `list_lit` head dispatch before the generic
application path. (2) Map keys may now be **strings** (`{"x" 5}`) as well as keywords, both
yielding field name `x` (new `_map_key_name` helper, strips the quotes/colon). (3)
`_kwd_accessed_params` extended so a param read via `(get p :k)` also types as `Axon`.
Substrate-verified fixture `map_get` = 13 (`(+ (get p :x) (get p :y))`; `(sum2 {"x" 6 "y"
7})`); Clojure suite 24/24 compile-AND-run. Numeric/symbol keys + recursive-body maps remain
later items.

## 2026-06-15: Rust frontend — nested/non-tail `match` in expression position (Phase 3)

Phase 3 transpiler-backlog increment. A variant `match` may now appear nested in a larger
expression (e.g. `100 + match e { … }`), not only as the function tail. `_lower_match_stmts`
was parameterized with `tagv`/`valv` so a hoisted nested match gets collision-free binding
locals (`_vtag_hN`, `_val_hN_i`); `_hoist_enum_constructions` (the post-order construction
hoist) was extended to also hoist `match_expression` nodes — emitting their binding
statements to the prelude and registering `node.id → blend-expr` in `_ARG_HOIST`, so
`_lower_expr` substitutes the blend at the use site. The existing function-tail match path
is unchanged (default `tagv`/`valv` reproduce the prior `_vtag`/`_val{i}` output byte-for-byte;
verified by `enum_match` still passing). Substrate-verified fixture `nested_match` = 202
(`evalE e = 100 + match e { Lit n => n, Neg n => 0 - n }`; `evalE(Lit 7)` 107 + `evalE(Neg 5)`
95); Rust suite 24/24 compile-AND-run.

## 2026-06-15: Elixir frontend — struct literals → axons (Phase 3)

Phase 3 transpiler-backlog increment, building on Elixir maps→axons. A struct literal
`%Name{x: a, y: b}` parses as a `map` node with an extra `struct` child (the nominal
type) plus the usual `map_content` — so it already flowed through `_map_fields`/the
maps→axons path, lowering to the same named-field axon with the struct alias dropped (a
struct is structurally an axon, the Rust `struct`-as-named-field-axon shape). Made the
behavior intentional/documented in `_map_fields` and locked it in with a substrate-
verified fixture `struct_axon` = 13 (`sum2(%Point{x: 6, y: 7})`); Elixir suite 22/22
compile-AND-run. The `%{"k" => v}` arrow form and non-atom keys remain later items.

## 2026-06-15: F# frontend — return-type annotations (Phase 3)

Phase 3 transpiler-backlog increment. F# return-type-annotated definitions (`let f (a:
int) (b: int) : int = …`) now lower with the annotated return type instead of falling
through unrecognized. These parse as a `value_declaration_left` (not the
`function_declaration_left` of the un-annotated form), and the curried-type grammar nests
params + the `: R` annotation differently per arity — so `_extract_value_decl_left` walks
the subtree structurally: every `paren_pattern` (document order) is one `(x: T)` param,
and the return type is the `simple_type` inside a param-and-return `typed_pattern` (or a
direct `simple_type` child for a bare `let x : R = …` value). `_lower_defn` refactored to a
unified path that threads the return type `R` into the emitted signature (was hardcoded
`int`). Two arity-dependent nesting bugs (params dropped for the 1- and 2-param forms)
caught by reading the lowered output before claiming done. Substrate-verified fixture
`return_type` = 13 (`let add (a: int) (b: int) : int = a + b`; `add 6 7`); F# suite 18/18
compile-AND-run. Return-type threading also exercised on `float` (`scale … : float`).

## 2026-06-15: Haskell frontend — `data` ADTs → tagged axons (Phase 3, variant tier)

Phase 3 transpiler-backlog increment — the full variant pattern for Haskell, ported from
the Rust enum machinery. A `data T = C1 a | C2 b | …` prepass (`_register_data_types`)
registers each constructor as `_VARIANTS[Ci] = (T, tag, arity)` and `T` in `_DATATYPES`
(so `_map_type` maps it to `Axon`). Value construction `C arg…` (statement-shaped) hoists
to a prelude `Axon _ahN; _ahN.add("_tag", tag); _ahN.add("_val{i}", arg); …` via the
`_ARG_HOIST` node-id mechanism (`_hoist_constructions`, which carefully skips
case-alternative PATTERNS and the head constructor of an `apply` — two bugs caught by
reading the lowered output before claiming done). A `case scrut of (C x) -> r; …` at the
function tail lowers to `_vtag`/`_val{i}` binding locals + a nested defuzz blend on `_vtag
== tag` with payload-name substitution (`_lower_case_stmts`, the Rust `_lower_match_stmts`
shape). New helpers `_register_data_types`/`_construction`/`_emit_construction`/
`_hoist_constructions`/`_lower_case_stmts` + `_VARIANTS`/`_DATATYPES`/`_ARG_HOIST`/
`_AH_COUNTER` reset per `lower()`. Non-variable scrutinees, nested payload patterns, and
`case` in non-tail position are explicit later items. Substrate-verified fixture `data_adt`
= 2 (`data Expr = Lit Int | Neg Int`; `evalE (Lit 7) + evalE (Neg 5)` = 7 + (−5), matching
the OCaml/Rust `enum_match` ground truth); Haskell suite 18/18 compile-AND-run.

## 2026-06-15: Clojure frontend — maps → axons (Phase 3, data-structure tier)

Phase 3 transpiler-backlog increment — the data-structure tier for Clojure, mirroring
the Elixir maps→axons shape committed the same day. A map literal `{:x a :y b}` (keyword
keys) hoists to a prelude temp `Axon _ahN; _ahN.add("x", a); …` via the `_ARG_HOIST`
node-id mechanism (axon construction is statement-shaped, can't lower inline). Keyword
access `(:x p)` — a keyword in list-head position — lowers to `realvec(p.item("x"))`; a
param read via `(:k p)` types as `Axon` instead of the default `number`
(`_kwd_accessed_params`). New helpers `_kwd_name`/`_map_fields`/`_hoist_maps`/
`_kwd_accessed_params` + `_ARG_HOIST`/`_AH_COUNTER` reset per `lower()`. Non-keyword keys,
`(get m :k)` access, and maps in recursive bodies are explicit later items. Substrate-
verified fixture `map_axon` = 13 (`(defn sum2 [p] (+ (:x p) (:y p)))`; `(sum2 {:x 5 :y
8}))`); Clojure suite 22/22 compile-AND-run.

## 2026-06-15: Elixir frontend — maps → axons (Phase 3, data-structure tier)

Phase 3 transpiler-backlog increment — the first data-structure tier for Elixir. Ported
the Rust/OCaml arg-hoist machinery: a map literal `%{x: a, y: b}` (atom-key shorthand)
hoists to a prelude temp `Axon _ahN; _ahN.add("x", a); …` with `_ARG_HOIST[node.id]`
carrying the temp name to where the map appeared (axon construction is statement-shaped,
can't lower inline). Field read `p.x` — which parses as a zero-arg `call` wrapping a
`dot` — lowers to `realvec(p.item("x"))` (the Rust struct-access shape); a param read via
dot-access types as `Axon` instead of the default `number` (`_dot_accessed_params`). New
helpers `_map_fields`/`_hoist_maps`/`_dot_accessed_params` + `_ARG_HOIST`/`_AH_COUNTER`
state reset per `lower()`. Scoped to the single bare-param `_lower_def` path; the `%{"k"
=> v}` arrow form, non-atom keys, and maps in multi-clause/recursive bodies are explicit
later items. Substrate-verified fixture `map_axon` = 13 (`def sum2(p), do: p.x + p.y`;
`sum2(%{x: 5, y: 8})`); Elixir suite 20/20 compile-AND-run.
...
## 2026-06-13 — daily audit: clean (end-to-end, including semantic await tests)

71 .su compiled, 0 user-program + 0 runtime-prelude leaks (`experiments/substrate_leak_sweep.py`); `scripts/check_promise_await_fit_to_spec.py` returns `EXIT=0` with `[2/2] regression tests  PASS (4/4 expected)`. **Today differs from 06-10/06-11/06-12:** the 2 semantic-preservation tests (`test_await_semantics_preserved_{torch,numpy}`) DID run after installing the Ollama daemon binary (`apt-get install zstd && bash <(curl -fsSL https://ollama.com/install.sh)`) and pulling `nomic-embed-text` — both PASS to 3 places against the fixture's expected 3.0. End-to-end promise/await verification is no longer un-run — it's CLEAN. `await_value` body at `codegen_pytorch.py:935–966` is still the spec-compliant `return self.value(p)` reduction per Audit REAL LEAK #3 (FIXED 2026-05-17, kept) — no `for _ in range(100)` / `if self.isPending` signature anywhere. Audit.md REAL LEAK #1–#10 all still FIXED/NOT-A-LEAK at cited codegen sites; #4 (`_TorchVSA.loop` fixed-T unroll) reconfirmed at `codegen_pytorch.py:2988` (`for _t in range(max_iters)` is structural-index unroll over a soft-halt eigenrotation, no `.item()`/`float()`/host branch on data). Spot-checked every `_emit(...float(...))` / `_emit(...item()...)` / `_emit(...for ... in range(...))` hit against Audit.md taxonomy + `experiments/substrate_leak_sweep.py::_PRELUDE_LEAK_EXEMPT_METHODS` — every site is documented BORDERLINE (Promise inspectors, `make_real`/`make_truth`/`make_char` literal entry, `array_from_literal`, `load_matrix` CSV text parse, JS-interop equality/promotion + `_js_str_cmp` per CLAUDE.md carve-out, `_argmax_cosine` / `_vector_map_lookup` terminal index at `:3073` / `:3106`, `string_to_python` substrate→host terminal decode at `:2617–2622`, `defuzzify_trit` structural-iters loop at `:2658` per Audit #4, `digit_array_add` structural-N loop at `:2740` per Audit #4) or LEGITIMATE (compile-time constants PI/TAU at `:324–325`); no new sites since 2026-06-12. 14 open-question dossiers in `planning/open-questions/` + `sutra-spec/open-questions.md` cross-checked: README verdict table unchanged from 2026-05-28 pruning; `axon-string-filler-roundtrip.md` still marked RESOLVED 2026-06-08 inline (kept as record, not "resolved-elsewhere drift"); `concurrency-and-monads.md` / `no-null.md` / `codegen-v1-feature-coverage.md` / `defuzzify-iteration-formula.md` spot-checked against `sutra-spec/concurrency.md` / `types.md:255–289` / `equality-and-defuzzification.md` — all still genuinely open (spec keeps both `var X:TYPE;` and `wait` forms with documented use cases per 2026-04-22/04-25, but whether to forbid the uninitialized form remains the same not-yet-decided open tail; the polarization rule choice is unsettled in the spec); spec-index strikethroughs (`types §"scalars as results"`, `control-flow §"loop unroll for"`, `binding §"surface syntax"`) unchanged. No code regression detected in any scope. (Setup note: `numpy`, `torch`, `pytest`, `ollama` Python pkg, and Ollama daemon binary absent on session start; all installed for this run.)

## 2026-06-14 — daily audit: clean (end-to-end, including semantic await tests)

71 .su compiled, 0 user-program + 0 runtime-prelude leaks (`experiments/substrate_leak_sweep.py`); `scripts/check_promise_await_fit_to_spec.py` returns `EXIT=0` with `[2/2] regression tests  PASS (4/4 expected)`. Same shape as 06-13: after installing `numpy`, `torch`, `pytest`, the `ollama` Python pkg, and the Ollama daemon binary (`apt-get install zstd && bash <(curl -fsSL https://ollama.com/install.sh)` then `ollama serve` + `ollama pull nomic-embed-text`), the 2 semantic-preservation tests (`test_await_semantics_preserved_{torch,numpy}`) DID run and PASS to 3 places against the fixture's expected 3.0; end-to-end promise/await verification is CLEAN, not partial. `await_value` body at `codegen_pytorch.py:935–966` is still the spec-compliant `return self.value(p)` reduction per Audit REAL LEAK #3 (FIXED 2026-05-17) — no `for _ in range(100)` / `if self.isPending` signature anywhere; the prior body is referenced only in the leak-history docstring (lines 937–963, phrased without the literal old signature so the sweep doesn't false-positive). Audit.md REAL LEAK #1–#10 all still FIXED/NOT-A-LEAK at cited codegen sites; #4 (`_TorchVSA.loop` fixed-T unroll) reconfirmed at `codegen_pytorch.py:2988` (`for _t in range(max_iters)` is structural-index unroll, body is `self._step(state, rotation, target, halted, k, threshold)` — tensors in/out, soft halt via sigmoid, no `.item()`/`float()`/host branch on data). Spot-checked every `_emit(...float(...))` / `_emit(...item()...)` / `_emit(...for ... in range(...))` hit against Audit.md taxonomy + `experiments/substrate_leak_sweep.py::_PRELUDE_LEAK_EXEMPT_METHODS` — every site is documented BORDERLINE (Promise inspectors lines 903–933, `make_real`/`make_truth`/`make_char` literal entry, `array_from_literal`, `load_matrix` CSV text parse, JS-interop equality/promotion + `_js_str_cmp` per CLAUDE.md carve-out, `_argmax_cosine` / `_vector_map_lookup` terminal index, `string_to_python` substrate→host terminal decode, `defuzzify_trit` structural-iters unroll per Audit #4, `digit_array_add` structural-N loop per Audit #4) or LEGITIMATE (compile-time constants PI/TAU at `:324–325`); no new sites since 2026-06-13. 14 open-question dossiers in `planning/open-questions/` + `sutra-spec/open-questions.md` cross-checked: README verdict table unchanged from 2026-05-28 pruning; `axon-string-filler-roundtrip.md` still marked RESOLVED 2026-06-08 inline (kept as record, not "resolved-elsewhere drift"); `2026-06-13-sutra-to-thrml-mapping.md` is an active exploration loop (Emma 2026-06-13) and is correctly outside the resolved/stale axis; spec-index strikethroughs (`types §"scalars as results"`, `control-flow §"loop unroll for"`, `binding §"surface syntax"`) unchanged. No code regression detected in any scope.
...
## 2026-06-18 — daily audit: clean (end-to-end, including semantic await tests)

73 .su compiled, 0 user-program + 0 runtime-prelude leaks (`experiments/substrate_leak_sweep.py`); `scripts/check_promise_await_fit_to_spec.py` returns `EXIT=0` with `[2/2] regression tests  PASS (4/4 expected)`. Same shape as 06-13/06-14/06-17: after installing `numpy`, `torch`, `pytest`, the `ollama` Python pkg, the `sutra_compiler` editable install, and the Ollama daemon (`apt-get install zstd && bash <(curl -fsSL https://ollama.com/install.sh)` then `ollama serve` + `ollama pull nomic-embed-text`), the 2 semantic-preservation tests (`test_await_semantics_preserved_{torch,numpy}`) DID run and PASS to 3 places against the fixture's expected 3.0; end-to-end promise/await verification is CLEAN. `test_no_host_readout.py` (5/5) + `test_substrate_leak_sweep.py` (1/1) + `test_await_substrate_pure.py` (4/4) = 10/10 PASS. `await_value` body at `codegen_pytorch.py:935–966` is still the spec-compliant `return self.value(p)` reduction per Audit REAL LEAK #3 (FIXED 2026-05-17) — no `for _ in range(100)` / `if self.isPending` signature anywhere; the prior body is referenced only in the leak-history docstring (lines 937–963, phrased without the literal old signature so the sweep doesn't false-positive). Audit.md REAL LEAK #1–#10 all still FIXED/NOT-A-LEAK at cited codegen sites; #4 (`_TorchVSA.loop` fixed-T unroll) reconfirmed at `codegen_pytorch.py:3011–3041` (`for _t in range(max_iters)` is structural-index unroll over `self._step(state, rotation, target, halted, k, threshold)` — tensors in/out, soft halt via sigmoid on `halted`, no `.item()`/`float()`/host branch on data); #9 (`eq`/`eq_synthetic`) reconfirmed at `codegen_pytorch.py:2867–2888` (scatter `out[self.semantic_dim + self.AXIS_TRUTH] = cos`, no `make_truth(float(cos.item()))` — that form only appears in the explanatory docstring at line 2878); #10 (`_select_softmax`) reconfirmed at `codegen_pytorch.py:64–90` (grad-tracked scores go through `_torch.stack`, raw numbers still through `_torch.as_tensor`). 14 open-question dossiers in `planning/open-questions/` + `sutra-spec/open-questions.md` cross-checked: README verdict table unchanged from 2026-05-28 pruning; `axon-string-filler-roundtrip.md` still marked RESOLVED 2026-06-08 inline (kept as record per Emma; resolution mirrored authoritatively in `sutra-spec/axons.md:285–294`, so not "resolved-elsewhere drift" in the audit sense); `2026-06-13-sutra-to-thrml-mapping.md` is an active exploration loop (Emma 2026-06-13) and is correctly outside the resolved/stale axis; spec-index strikethroughs (`types §"scalars as results"`, `control-flow §"loop unroll for"`, `binding §"surface syntax"`) unchanged. No code regression detected in any scope.

## 2026-06-18 — sutra-from-haskell: nested CONSTRUCTOR `case` patterns `Outer (Inner a b) c -> …` ship (== 16)

Phase 6, Haskell item — the case-arm companion to the nested-ctor-let. A `case` arm with a nested
constructor payload (`case w of Outer (Inner a b) c -> a+b+c`) previously surfaced UNSUPPORTED
("non-variable payload pattern"). The apply-arm in `_lower_case_stmts` now splits: an all-variable
payload list uses the existing shared `int _val{i}` locals (unchanged — flat variant case
unregressed); a payload list with a NESTED ctor uses `_collect_hs_ctor_paths` to flatten the whole
pattern to `_val{i}` key-paths, reads each leaf through an `Axon` temp per non-leaf prefix (emitted
into the case prelude, reading from the scrutinee), and keeps the OUTER constructor tag as the arm
test. `_val0`/`_val1` keys read clean at dim 50 (measured == 16), as in the let case.

Fixture `nested_ctor_case` (`data Inner=Inner Int Int; data Outer=Outer Inner Int; f w=case w of Outer
(Inner a b) c -> a+b+c; f (Outer (Inner 5 8) 3)`) runs == **16**. Haskell suite: 42 passed (lower-only
...
## 2026-06-24 — daily audit: clean (end-to-end, including semantic await tests)

79 .su compiled, 0 user-program + 0 runtime-prelude leaks (`experiments/substrate_leak_sweep.py`); `scripts/check_promise_await_fit_to_spec.py` returns `EXIT=0` with `[2/2] regression tests  PASS (4/4 expected)`. End-to-end CLEAN: after installing `numpy`, `torch`, `pytest`, the `ollama` Python pkg, the `sutra_compiler` editable install, plus `sentence-transformers`+`einops` for the in-process embedding backend (the post-2026-06-22 default — Ollama daemon install errored 404 in this proxied container, so `SUTRA_EMBED_BACKEND=hf` routed through `nomic-ai/nomic-embed-text-v1.5`), the 2 semantic-preservation tests (`test_await_semantics_preserved_{torch,numpy}`) DID run and PASS to 3 places against the fixture's expected 3.0; promise/await semantic verification is no longer un-run. `test_no_host_readout.py` (5/5) + `test_substrate_leak_sweep.py` (1/1) + `test_await_substrate_pure.py` (4/4) = 10/10 PASS. `await_value` body at `codegen_pytorch.py:1034` is still the spec-compliant `return self.value(p)` reduction per Audit REAL LEAK #3 (FIXED 2026-05-17) — verified by direct codegen probe: emission contains neither `for _ in range(100)` nor `if self.isPending`; the prior body is referenced only in the leak-history docstring (phrased without the literal old signature so the sweep doesn't false-positive). Audit.md REAL LEAK #1–#11 all still FIXED/NOT-A-LEAK at cited codegen sites; #4 (`_TorchVSA.loop` fixed-T unroll) reconfirmed at `codegen_pytorch.py:3636` (structural-index unroll over `self._step(...)` — tensors in/out, soft halt via sigmoid, no `.item()`/`float()`/host branch on data); #9 (`eq`/`eq_synthetic`) reconfirmed at `codegen_pytorch.py:3478`/`:3503` (scatter `out[self.semantic_dim + self.AXIS_TRUTH] = cos`, no `make_truth(float(cos.item()))` — that form only appears in the explanatory docstring at line 3487); #10 (`_select_softmax`) reconfirmed at `codegen_pytorch.py:93`; #11 (`js_strict_eq` host-arithmetic) reconfirmed at `codegen_pytorch.py:2982` (scatter into `AXIS_TRUTH` directly, grad-tracked). Spot-checked every `.item()` / `float(` hit in `codegen_pytorch.py` against `experiments/substrate_leak_sweep.py::_PRELUDE_LEAK_EXEMPT_METHODS` allowlist — every site is documented BORDERLINE (Promise inspectors, `make_real`/`make_truth`/`make_char` literal entry, `array_from_literal`/`array_length`/`array_get`/`array_map`/`array_filter`, `load_matrix` CSV text parse, JS-interop equality/promotion + `_js_str_cmp` per CLAUDE.md carve-out, `_argmax_cosine`/`_vector_map_lookup` terminal index, `string_to_python` substrate→host terminal decode, RAM address-decode I/O wire) or LEGITIMATE (compile-time constants PI/TAU); no new sites since 2026-06-18. 16 open-question dossiers in `planning/open-questions/` + `sutra-spec/open-questions.md` cross-checked: README verdict table (14 entries) unchanged from 2026-05-28 pruning; `axon-string-filler-roundtrip.md` still marked RESOLVED 2026-06-08 inline (Emma kept as record; resolution mirrored authoritatively in `sutra-spec/axons.md:285–294`, so not "resolved-elsewhere drift" in the audit sense); `2026-06-13-sutra-to-thrml-mapping.md` is an active exploration loop (Emma 2026-06-13) and is correctly outside the resolved/stale axis; spec-index strikethroughs unchanged. No code regression detected in any scope.
