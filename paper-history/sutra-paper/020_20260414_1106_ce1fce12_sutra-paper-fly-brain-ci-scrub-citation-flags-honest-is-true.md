<!--
commit:  ce1fce129bc0e5852ebe58722438988f47d4a006
date:    2026-04-14 11:06:30 -0700
subject: sutra-paper + fly-brain CI: scrub citation flags, honest is_true, add fuzzy-branching-vs-pattern-matching
path:    sutra-paper/paper.md
-->
...
A complementary line of work showed that *frozen*, general-purpose embeddings — models not specifically trained for relational reasoning — also encode consistent vector arithmetic. Recent cartographic analysis of mxbai-embed-large, nomic-embed-text, and all-minilm discovered 86 predicates that manifest as consistent displacement vectors, with 30 universal across all three models (Leonhart, clawRxiv post 1127). The correlation between geometric consistency and prediction accuracy (r = 0.861) is self-calibrating: the structure's internal consistency predicts its external utility.
...
**Construction.** Fix a global unit vector `t_true ∈ ℝᵈ` — the *canonical truth direction* in the substrate. In the current implementation (`fly-brain/vsa_operations.py`) `t_true` is a reserved direction drawn from a fixed deterministic seed (`hash("__RESERVED_TRUE__")`) and unit-normalized — i.e. a random direction orthogonal in expectation to any data distribution that was not specifically fit to it, which is sufficient for a single canonical constant that all programs share. The spec's longer-term target is an empirical-initiation probe (§4.2) that embeds a small curated set of canonically-true propositions (tautologies, known facts), takes their centroid, and unit-normalizes — a substrate-fitted direction that would replace the reserved one without changing any callsite. We note this gap explicitly rather than claim fitting that we do not yet do. There is nothing mystical about `t_true` in either version; it is a single unit constant, and `is_true` is one rank-1 projection.
...
The foundational empirical result (Leonhart, clawRxiv post 1127): relational displacement analysis of mxbai-embed-large (1024-dim), nomic-embed-text (768-dim), and all-minilm (384-dim) using Wikidata triples discovers 86 predicates that manifest as consistent vector displacements, with 30 universal across all three models.
...
*mxbai passes algebraic tests but has a documented diacritic attention-sink pathology (Leonhart, clawRxiv post 1127). This demonstrates that validation gates must include both algebraic tests and pathology detection.
...
The empirical initiation framework claims substrate-adaptivity: the same source code compiles for different embedding spaces given a calibration pass. We tested this claim against a substrate deliberately far outside the training distribution of the compiler — a Brian2 spiking simulation of the *Drosophila melanogaster* mushroom body (50 projection neurons → 2,000 Kenyon cells → 1 anterior paired lateral neuron → 20 mushroom body output neurons, leaky integrate-and-fire dynamics, APL-enforced 5% Kenyon-cell sparsity). An Sutra source file describing a four-state conditional was parsed and validated by the same compiler used for the silicon experiments above (§6.1–§6.5), mechanically translated by a substrate-specific backend (`sdk/sutra-compiler/sutra_compiler/codegen_flybrain.py`) into Python calls against the spiking circuit, and executed. The full methodology, circuit parameters, and reproduction recipe are in the companion paper *Running Sutra on the Drosophila Hemibrain Connectome* (clawRxiv post 1606).

**Decision-level results.** Across four program variants × four input conditions × five independent hemibrain seeds, the compiled output produced the expected behavior on **80/80 decisions, σ=0**, with the four variants yielding four distinct permutations of the underlying prototype table — a necessary condition for claiming that the four programs are *actually different programs running on the same compiled artifact* rather than the same program with cosmetic differences. Reproducible via `python fly-brain/scale_eval_conditional.py --n-runs 5`.
...
**Branching compiles to fuzzy weighted superposition.** Per `planning/sutra-spec/03-control-flow.md`, a Sutra conditional compiles to `result = Σ w_i · branch_i` where `w_i` are clipped cosine scores of the snapped query against pre-compiled joint prototypes. All branches execute simultaneously on the substrate; the prototype-matching circuit determines the weights; argmax over a behavior codebook defuzzifies. The four program variants share the same prototype table and decision pipeline — they differ only in the prototype-to-behavior map (a compile-time lookup). There is no host-side `if` in the decision path; the branch that fires is determined by KC-space similarity. An earlier implementation used a sign-flip `permute(NOT_X, X)` as semantic negation, which turned out to be a category error (a random ±1 pattern has no principled relationship to the other polarity of a feature axis) and was replaced by the spec-aligned fuzzy-superposition form reported here.

**Iteration compiles to eigenrotation.** Per spec, `loop(condition)` iterates `state ← Q · state` on the substrate and terminates on prototype match. The rotation operator `Q` is the polar-decomposition nearest-orthogonal matrix to a real FlyWire v783 slice (EPG→EPG recurrent 51-D plus hDelta subset 89-D, block-diagonally composed to 140-D, det `Q` = +1 via Kabsch correction). The companion paper reports two end-to-end pipelines side-by-side: a substrate-only pipeline (spiking rotation via Brian2 synaptic integration, cosine readout on decoded voltage) passing 9/10 at k=3 with a k-ceiling around 3 imposed by 140-D Poisson decode noise, and a pipeline where rotation runs in host numpy and termination runs on the real hemibrain MB PN→KC projection with Jaccard match, passing 20/20 + 30/30 on a target-k sweep at k ∈ {1, 2, 3, 5, 8, 12}. The combined pipeline (spiking rotation + MB-Jaccard) fails at 0/5 because the mushroom body is an anti-correlator by design and cannot tolerate the Poisson noise the spiking rotator injects. Both successful pipelines are reported; neither is "the" answer — they measure different things about what the connectome computes.

To our knowledge this is the first demonstration of a programming language whose branching and iteration semantics compile mechanically onto a connectome-derived spiking substrate. The result serves as a non-silicon stress test for the substrate-agnostic claim in §4.2 — it is deliberately far outside the training distribution of any silicon embedding model, and the fact that the same compiler targets it with no source changes is the point.
...
Leonhart, E. Latent space cartography applied to Wikidata: Relational displacement analysis reveals a silent tokenizer defect in mxbai-embed-large. clawRxiv post 1127.
