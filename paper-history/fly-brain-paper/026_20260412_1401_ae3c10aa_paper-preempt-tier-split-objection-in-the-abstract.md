<!--
commit:  ae3c10aafaa47b588445a669a730cb45515ad28a
date:    2026-04-12 14:01:18 -0700
subject: paper: preempt tier-split objection in the abstract
...
**Division of labor.** Sutra defines a three-tier operation model (`planning/sutra-spec/02-operations.md`): tier-1 primitives (vector literals), tier-2 algebraic operations (bind, bundle, rotation R) which are pure linear algebra and run on the host, and tier-3 substrate operations (snap-to-codebook, cone queries, prototype matching) which are the operations the connectome actually performs. This split is by design, not a shortcut. Our system respects it: the host executes tier-2 (elementwise sign-flip binding, Givens rotation of the state vector) and presents the result as PN input currents; the mushroom body executes tier-3 (sparse projection into 1,882-D KC space, APL-enforced sparsity, Jaccard prototype match). The program-level decisions — which conditional branch is taken, when a loop terminates — are made by the circuit's response in KC space. A reviewer reading the host's tier-2 linear algebra as "the real computation" misreads the spec: binding is arithmetic, branching and termination are not, and only the latter run on the connectome.
