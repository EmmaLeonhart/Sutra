...
np.savez_compressed('data/embeddings_clean.npz', vectors=emb)

# Verify
data2 = np.load('data/embeddings_clean.npz')
emb2 = data2['vectors']
...
# Replace
import os, shutil
shutil.move('data/embeddings.npz', 'data/embeddings_backup.npz')
shutil.move('data/embeddings_clean.npz', 'data/embeddings.npz')
print("Replaced embeddings.npz with clean version.", flush=True)

# Final verify
data3 = np.load('data/embeddings.npz')
