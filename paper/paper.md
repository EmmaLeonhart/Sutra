...
Only before/after accuracy was logged for the compiled run, so no
per-epoch curve is plotted (fabricating one is not an option).
...
| Differentiable training | 3.6 | `differentiable_training_compiled.py --batched` | 3 seeds × 30 epochs (K=5, 50 words) | nomic-embed-text (frozen) | Adam, lr=0.01 | 0–2 |
