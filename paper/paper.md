...
To answer reviewer requests for quantitative neuro-symbolic
benchmarks, we ran the role-filler-record decode task in Sutra
and TorchHD on the same hardware (CPU, 768-dim vectors, 50-call
warmup, 1000-call timed steady state). TorchHD is the closest
apples-to-apples baseline because it is also VSA-on-PyTorch and
implements the same task line-for-line. Scallop and DeepProbLog
are different paradigms (probabilistic logic programming with
neural facts) and have no shared workload that's a fair head-
to-head; we discuss this scope choice below.

| system | compile (one-time) | cold call | steady-state |
|---|---:|---:|---:|
| Sutra (no torch.compile) | 1957 ms | 5651 µs | 7681 ± 1621 µs |
| Sutra (`SUTRA_TORCH_COMPILE=1`) | 2717 ms | 5473 µs | 5678 ± 612 µs |
| TorchHD | – | 872 µs | 525 ± 228 µs |

**Sutra is currently ~12× slower than TorchHD per call.** This
is a real runtime overhead gap, not a measurement artifact.
Both systems dispatch the same PyTorch tensor ops underneath;
the difference is scaffolding the Sutra runtime threads through
every call (per-axis canonical synthetic dimensions, slot-state
accumulator, halt-cum threading, list-of-(vec, str) codebook
lookup). `torch.compile` wrapping shaves ~25% off but does not
close the gap — the compiled code is still a Python function
calling many small tensor ops, not one fused kernel.

This is a runtime-optimization gap, not an architectural one.
The compiler's reduction to a fused tensor-op graph happens at
compile time (beta reduction, rotation precomputation, fixed-
depth loop unroll); the runtime *executes* that graph through
Python op-by-op rather than as a single traced kernel. Closing
the gap is a runtime-engineering task — per-op fast paths, lazy
slot allocation, removing halt-cum from non-loop functions, and
single-block `torch.compile` over the whole program — not a
language redesign.

What Sutra retains over TorchHD is *what can be expressed*: a
typed source language, differentiable fuzzy logic via Lagrange-
interpolated Kleene connectives (§1.2), soft-halt RNN cells
(§3.3), compile-time string codebooks (§3.4), substrate-purity
guarantees (§4). The benchmark above is restricted to the
intersection of what both systems can do; on tasks Sutra
expresses but TorchHD does not (a knowledge-graph traversal
loop with halt condition, fuzzy dispatch over a category
hierarchy), there is no comparable TorchHD program.

**Why no Scallop / DeepProbLog number.** Scallop has no PyPI
package — its install is a multi-hour Rust source build (cargo
build -p scallopy), and we judged that a same-session number
mattered more than spinning up an isolated container.
DeepProbLog and LTN install cleanly but are probabilistic logic
programming and FOL-constraint training systems, respectively;
their workloads (SLG resolution over Prolog rules, FOL-
constrained neural-network training) have no fair-comparison
overlap with Sutra's compositional VSA encode/decode. A
contrived benchmark would measure each system at something it
does not natively do; we declined.

Reproduction: `python experiments/sutra_vs_torchhd_latency.py`
(add `SUTRA_TORCH_COMPILE=1` for the compile-wrapped row); raw
JSON in `experiments/sutra_vs_torchhd_latency_results.json`.
