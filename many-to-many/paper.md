...
The orthogonal projection step (part 2) is a well-known technique in embedding debiasing (Bolukbasi et al., 2016; Ravfogel et al., 2020). Our contribution is demonstrating that projection alone is insufficient for structured matching — the directional selection step (part 1) is the critical differentiator. In our experiments (Section 4), control-only matching (parts 2+3) wins on MAP against naive cosine in 1/9 experiments, while the full three-part primitive (parts 1+2+3) wins against naive cosine in 8/9 experiments under MAP (one all-minilm/Animals regression of 0.005 points, within re-run noise) and against projection-alone in 9/9. The selection component directs the query toward the desired dimension rather than merely removing the unwanted one.
...
**Metrics:** Mean Average Precision (MAP — measures the quality of the full ranking of correct items, not just the first hit), Precision@k (k = number of correct items), and mean rank of correct items. We do not report Mean Reciprocal Rank as a discriminating metric here because it saturates at 1.000 for all three methods in all nine experiments — every method places at least one correct candidate at rank 1, so MRR cannot distinguish the methods on these datasets. MAP and P@k capture the later-rank structure where the methods actually diverge.
...
**Perfect precision (1.000) achieved in 5/9 experiments with the full method vs 0/9 with naive cosine.** The five perfect-precision cases are all three Occupations experiments plus mxbai-embed-large and nomic-embed-text on Animals. The four non-perfect cases split cleanly: the three Countries experiments plateau at P@23 = 0.826–0.913 because 23 correct candidates sit in a dense cosine neighborhood that structured matching tightens but does not perfectly separate; the all-minilm Animals case ties naive cosine at 0.933 because naive is already nearly perfect there (naive MAP 0.988, the one case where the three-part primitive marginally regresses).

### 4.3 Analysis

**The directional selection component is the key differentiator.** Control-only matching (equivalent to Bolukbasi-style debiasing projection) barely improves over naive cosine under MAP — it wins in only 1 of 9 experiments (all-minilm on Countries), and in one case (nomic on Animals) actually hurts MAP (0.884 vs. naive 0.919). Adding the directional selection step (part 1) converts this to 9 of 9 wins against projection-alone and 8 of 9 wins against naive cosine under MAP. This is not a contradiction of the debiasing literature: Bolukbasi et al. (2016) showed that projection successfully removes gender bias from *analogy tasks*, where the evaluation metric is specifically sensitive to the projected dimension. In our matching task, removing one confounder barely changes the ranking because many other dimensions still dominate the cosine score. Directional selection reweights the score toward the desired dimension, which is the missing step that makes the difference.
...
**The animals dataset is the strongest test.** Here, the target direction (aquatic habitat) and control direction (phylogenetic class) have substantial overlap (target-control alignment 0.43–0.53), meaning the confounder is genuinely entangled with the signal. Despite this entanglement, the full primitive achieves perfect precision on two of three models (mxbai, nomic) and ties naive cosine at 0.933 on all-minilm — the one case where the confounder is already weaker than usual in the smaller 384-dim space, and structured matching neither helps nor hurts.
