...
## What This Is
A program that automatically generates a complex vector map from a large number of trajectories. Trajectories are paths through vector space connecting entities that are related by Wikidata triples. Almost everything in the system is a trajectory — the map IS the trajectories.

The output is a dense, structured map of an embedding space that can be compared across different embedding models. Given the same Wikidata input, two different models will produce different trajectory maps — and the differences reveal what each model "understands" about semantic structure.

## Goal
Establish semantic relationships as vector operations through trajectories, and classify regions of the embedding space by density:
...
The program systematically maps these regions by walking through Wikidata, generating trajectories, and tracking where collisions happen and where they don't.
...
3. Do a **random walk through Wikidata**, importing entities and filling in trajectories
4. Aggressively seek collisions — try to find where different things land in the same region
5. Keep a running tally of:
   - All trajectories and which ones appear to be common vectors for common relationships
   - Density achieved at each region of the embedding space
   - Which vector operations (if any) each trajectory manifests
6. The walk continues until the map is dense enough to draw conclusions

## Trajectories Are Real Objects, Not Results of Operations
Trajectories are not the results of vector operations. They are real mathematical objects that we identified — they simply exist as the line between two embedded texts that a Wikidata triple connects. They are data points, not conclusions.
...
A logical relationship like "man → woman" might hold as a consistent vector in English embedding spaces trained on Western corpora, but not in a Japanese model, or not in a model trained primarily on scientific text. Each embedding space needs to be mapped independently, using the same trajectory definitions, to discover what it actually encodes.

There will be thousands of trajectories. The program examines whether they cluster into consistent isosymbolic operations or scatter — and that tells you what the embedding space actually encodes and where it breaks down.
...
- **Not visualization.** The trajectory map is a structured data artifact, not a picture. It can be queried, compared, and analyzed programmatically.
